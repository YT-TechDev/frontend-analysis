        ...
        CandidateMode::InBody => {
            ...
            match shape {
                ...
                CandidateTokenShape::StartTag {
                    ...
                    effect: Some(CandidateEffect::InsertDiv),
                    next: None,
                }),
                CandidateTokenShape::EndTag {
                    name: CandidateElementName::Div,
                    ..
                } if open_div_depth > 0 => Ok(CandidateStep::Consume {
                    effect: Some(CandidateEffect::PopDiv),
                    next: None,
                }),
                CandidateTokenShape::EndTag {
                    name: CandidateElementName::Div,
                    ..
                } => Ok(CandidateStep::Ignore {
                    effect: CandidateEffect::UnmatchedDiv,
                }),
                CandidateTokenShape::EndTag {
                    name: CandidateElementName::Body,
                    ..
                ...
                CandidateTokenShape::EndOfFile { .. } => Ok(CandidateStep::Stop {
                    effect: (open_div_depth > 0).then_some(CandidateEffect::OpenElementAtEof),
                }),
                _ => Err(CandidateUnsupported::OutsideModelledCandidateCells),
            }
        }
...
impl CandidateSession {
    ...
    fn open_div_depth(&self) -> usize {
        self.open_elements
            .iter()
            .filter(|id| self.element_name(**id) == CandidateElementName::Div)
            .count()
    }
    ...
    fn assert_invariant(&self) {
        ...
        let valid = match names.as_slice() {
            ...
            [
                ...
                rest @ ..,
            ] => rest.iter().all(|name| *name == CandidateElementName::Div),
            _ => false,
        };
    ...
    fn insert_div(&mut self, shape: &CandidateTokenShape<'_>) {
        ...
        let CandidateTokenShape::StartTag {
            ...
            panic!("div insertion requires a div start tag")
        };
        let parent = self.current_node().expect("div insertion parent");
        let id = self.allocate(CandidateArenaKind::Element {
            name: CandidateElementName::Div,
            namespace: CandidateNamespace::Html,
...
fn candidate_gold(id: &str, source_id: u64) -> CandidateSemanticObservation {
    ...
    match id {
        "DV1" => CandidateSemanticObservation {
            ...
            identity_count: 5,
            checkpoint: checkpoint(CandidateMode::InBody, 0, 17, CandidateCompletion::Complete),
        },
        "DV2" => CandidateSemanticObservation {
            tree: shell(
                ...
                vec![element(
                    ...
                    CandidateElementName::Div,
                    authored_origin(source_id, (6, 11), (7, 10)),
                    vec![text(5, source_id, "x", &[(11, 12)])],
                )],
            ),
            diagnostics: vec![missing_body()],
            closures: vec![closure(source_id, 4, 3, (12, 18))],
            identity_count: 6,
            checkpoint: checkpoint(CandidateMode::InBody, 0, 18, CandidateCompletion::Complete),
        },
            ...
            tree: shell(
                ...
                vec![element(
                    ...
                    vec![element(
                        5,
                        CandidateElementName::Div,
                        authored_origin(source_id, (11, 16), (12, 15)),
                        vec![text(6, source_id, "x", &[(16, 17)])],
                    )],
                )],
            ),
            diagnostics: vec![missing_body()],
            closures: vec![
                closure(source_id, 5, 4, (17, 23)),
                closure(source_id, 4, 5, (23, 29)),
            ],
            identity_count: 7,
            checkpoint: checkpoint(CandidateMode::InBody, 0, 29, CandidateCompletion::Complete),
        },
        "DV4" => CandidateSemanticObservation {
            tree: shell(
                ...
                vec![
                    ...
                    element(
                        5,
                        CandidateElementName::Div,
                        authored_origin(source_id, (17, 22), (18, 21)),
                        vec![],
                    ),
                ],
            ),
            diagnostics: vec![missing_body()],
            closures: vec![
                closure(source_id, 4, 2, (11, 17)),
                closure(source_id, 5, 4, (22, 28)),
            ],
            identity_count: 6,
            checkpoint: checkpoint(CandidateMode::InBody, 0, 28, CandidateCompletion::Complete),
        },
        "DV5" => CandidateSemanticObservation {
            ...
            closures: vec![],
            identity_count: 4,
            checkpoint: checkpoint(CandidateMode::InBody, 0, 12, CandidateCompletion::Complete),
        },
        "DV6" => CandidateSemanticObservation {
            ...
            diagnostics: vec![
                ...
                CandidateDiagnostic {
                    code: CandidateDiagnosticCode::OpenOrdinaryElementAtEndOfFile,
                    trigger: CandidateTrigger::EndOfFile { index: 3 },
                    recovery: CandidateRecovery::StoppedParsingWithOpenElements,
                },
            ],
            closures: vec![],
            identity_count: 6,
            checkpoint: checkpoint(CandidateMode::InBody, 1, 12, CandidateCompletion::Complete),
        },
        "DV7" => CandidateSemanticObservation {
                ...
                vec![element(
                    ...
                    vec![
                        ...
                        element(
                            6,
                            CandidateElementName::Div,
                            authored_origin(source_id, (12, 17), (13, 16)),
                            vec![text(7, source_id, "b", &[(17, 18)])],
                        ),
            ...
            diagnostics: vec![missing_body()],
            closures: vec![
                closure(source_id, 6, 5, (18, 24)),
                closure(source_id, 4, 7, (25, 31)),
            ],
            identity_count: 9,
            checkpoint: checkpoint(CandidateMode::InBody, 0, 31, CandidateCompletion::Complete),
        },
        "DV8a" => CandidateSemanticObservation {
            ...
            checkpoint: checkpoint(
                ...
                CandidateCompletion::IncompleteUnsupported {
                    capability: CandidateUnsupported::TagWithAttributes,
        ...
        "DV8b" => CandidateSemanticObservation {
            ...
            checkpoint: checkpoint(
                ...
                6,
                CandidateCompletion::IncompleteUnsupported {
                    capability: CandidateUnsupported::SelfClosingTag,
                    trigger: authored_trigger(source_id, 1, (6, 12)),
                },
            ),
        },
        "DV9" => CandidateSemanticObservation {
            ...
            checkpoint: checkpoint(
                ...
                13,
                CandidateCompletion::IncompleteUnsupported {
                    capability: CandidateUnsupported::DivTagOutsideInBody,
                    trigger: authored_trigger(source_id, 2, (13, 18)),
                },
            ),
        },
        "DV10" => CandidateSemanticObservation {
            ...
            checkpoint: checkpoint(
                ...
                11,
                CandidateCompletion::IncompleteUnsupported {
                    capability: CandidateUnsupported::ShellInteractionWithOpenDiv,
                    trigger: authored_trigger(source_id, 2, (11, 18)),
                },
            ),
        },
            ...
            tree: shell(
                ...
                vec![
                    ...
                    element(
                        5,
                        CandidateElementName::Div,
                        authored_origin(source_id, (1, 6), (2, 5)),
                        vec![],
                    ),
                ],
            ),
...
fn depth_before(observation: &CandidateObservation, position: usize) -> usize {
    if position == 0 {
        ...
        observation.tokens[position - 1].open_div_depth_after
    }
}
...
fn observed_token(token: &HtmlToken) -> ExpectedToken {
    match token {
        ...
        HtmlToken::Tag(tag) => match tag.kind() {
...
#[test]
fn tokenizer_emits_exact_evidence_shape_for_all_canonical_fixtures() {
    ...
    for (id, expected_tokens) in expected {
        ...
        assert_eq!(run.is_incomplete(), id == "DV12", "{id}");
...
#[test]
fn candidate_configuration_and_pinned_branch_projections_are_exact() {
    assert_eq!(
    ...
    for name in [
        ...
        assert_eq!(
            name.permitted_at_in_body_eof(),
            matches!(
                name,
                CandidateElementName::Html | CandidateElementName::Body
            )
        );
    }
    assert_eq!(
        ...
        for (position, record) in observed.tokens.iter().enumerate() {
                ...
                && record.refusal().is_none()
            {
                ...
                assert_eq!(
                    ...
                    depth_before(&observed, position) + 1
                );
            }
        }
    }
...
fn matching_div_ends_record_exact_semantic_closure_relations() {
    for id in ["DV1", "DV2", "DV3", "DV4", "DV7", "DV13", "DV14"] {
        let run = run_for(id, 1);
        let observed = observe(&run);
        ...
        for closure in &observed.semantic.closures {
            let CandidateTrigger::Authored {
                ...
                panic!("matching div closure must be authored")
            };
            let HtmlToken::Tag(tag) = &run.tokens()[*index] else {
                panic!("closure trigger must identify an emitted tag")
            };
            assert_eq!(tag.kind(), HtmlTagKind::End);
            assert_eq!(tag.name().interpreted(), "div");
            assert_eq!(*trigger_evidence, evidence(tag.complete()));
...
#[test]
fn stray_div_end_is_diagnosed_ignored_and_creates_no_closure_or_identity() {
...
#[test]
fn character_data_keeps_parent_sensitive_contributions_and_coalescing_identity() {
...
fn dv_refusals_are_transactional_and_pin_exact_trigger_evidence() {
    for (id, capability, index, range, committed, depth) in [
        (
            "DV8a",
        ...
        let record = observed.tokens.last().expect("refused token record");
        assert_eq!(record.refusal(), Some(capability), "{id}");
        assert_eq!(record.mode_before, record.mode_after, "{id}");
        assert_eq!(record.committed_prefix_end, committed, "{id}");
        assert_eq!(record.open_div_depth_after, depth, "{id}");
        assert_eq!(record.trigger, authored_trigger(1, index, range), "{id}");
        assert_eq!(
        ...
        assert!(
            observed
                ...
                .closures
                .iter()
                .all(|closure| closure.trigger != record.trigger),
            "{id}"
        );
    }
}
...
fn candidate_provenance_retains_source_identity_and_keeps_domains_distinct() {
    for id in CANDIDATE_IDS {
        ...
        for closure in &observed.semantic.closures {
            if let CandidateTrigger::Authored { evidence, .. } = &closure.trigger {
                authored.push(evidence.clone());
            }
        }
        assert!(
        ...
        for closure in &observed.semantic.closures {
            ...
            collect_tree_evidence(&observed.semantic.tree, &mut tree_evidence);
            assert!(
                !tree_evidence
                    .iter()
                    .any(|tree_evidence| tree_evidence.range == closure_evidence.range),
                "{id}: end-tag evidence is not node origin or text contribution"
            );
        }
    }
}
...
fn semantic_identities_are_contiguous_and_relationships_are_not_storage_indices() {
    for id in CANDIDATE_IDS {
        let observed = observe_fixture(id, 1);
        let mut ids = Vec::new();
        collect_ids(&observed.semantic.tree, &mut ids);
        ids.sort();
        assert_eq!(
            ids,
            (0..observed.semantic.identity_count)
                .map(CandidateNodeId)
                .collect::<Vec<_>>(),
            "{id}"
        );
    ...
    assert_eq!(
        ...
        "semantic stack identities are layout-independent"
    );
    for semantic_id in &compact.open_elements {
        assert_ne!(
            compact.storage_index(*semantic_id),
            padded.storage_index(*semantic_id),
            "physical storage moves while semantic identity stays fixed"
        );
    }
}
...
fn lower_layer_incompleteness_is_never_upgraded_and_diagnostics_are_orthogonal() {
    ...
    assert_eq!(
        dv12.semantic.checkpoint.completion,
        CandidateCompletion::IncompleteLowerLayer
    );
    ...
    assert_eq!(
        observe_fixture("DV5", 1).semantic.checkpoint.completion,
        CandidateCompletion::Complete
    );
    assert_eq!(
        observe_fixture("DV6", 1).semantic.checkpoint.completion,
        CandidateCompletion::Complete
    );
}
    ...
    for id in CANDIDATE_IDS {
        ...
        for record in &observed.tokens {
            ...
            for (position, mode) in modes.iter().enumerate() {
                assert!(!modes[..position].contains(mode), "{id}");
            }
            assert!(modes.len() <= CandidateMode::ALL.len(), "{id}");
            if record.mode_before == CandidateMode::InBody {
                assert_eq!(record.dispatches.len(), 1, "{id}");
            }
        }
    }
}
...
fn generated_sequences_uphold_stack_closure_identity_and_completion_theorems() {
    ...
    for source in sources {
        ...
        let mut matching_ends = 0_usize;
        let mut stray_ends = 0_usize;
        for record in &observed.tokens {
            let Some(HtmlToken::Tag(tag)) = run.tokens().get(record.index) else {
                continue;
            };
            if tag.name().interpreted() != "div" {
                continue;
            }
            match tag.kind() {
                HtmlTagKind::Start => expected_depth += 1,
                HtmlTagKind::End if expected_depth > 0 => {
                    expected_depth -= 1;
                    matching_ends += 1;
                }
                HtmlTagKind::End => stray_ends += 1,
            }
            assert_eq!(record.open_div_depth_after, expected_depth, "{source:?}");
        }
        ...
        assert_eq!(
            observed.semantic.closures.len(),
            matching_ends,
            "{source:?}"
        );
        assert_eq!(
        ...
        assert!(
            observed
                ...
                .closures
                .iter()
                .all(|closure| closure.element.0 < observed.semantic.identity_count),
            "{source:?}"
        );
        assert_eq!(
...
fn candidate_semantics_and_authored_provenance_are_deterministic_across_source_ids() {
    for id in CANDIDATE_IDS {
        for source_id in [1_u64, 7, 4_242, u64::from(u32::MAX)] {
            assert_eq!(
                observe_fixture(id, source_id).semantic,
                candidate_gold(id, source_id),
...
fn historical_body_p_boundary_remains_exactly_unsupported_at_six_to_nine() {
    ...
    let CandidateCompletion::IncompleteUnsupported { trigger, .. } =
        observed.semantic.checkpoint.completion
    else {
        unreachable!()
    };
