...
use crate::css::value_qualification::{
    CssColumnFillQualificationOutcome, CssColumnFillUnsupportedReason, CssColumnFillValue,
...
fn expected_outcome(expected: ExpectedOutcome) -> CssColumnFillQualificationOutcome {
    match expected {
...
fn handwritten_direct_keyword_boundary_matches_pinned_wpt() {
    let result = qualify(
        ...
        concat!(
            ...
            r"d{column-fill:\61 uto;}",
            r"e{column-\66 ill:balance;}",
            "f{column-fill:none;}",
            "g{column-fill:auto balance;}",
            "h{column-fill:1;}",
            "i{column-fill:1px;}",
            "j{column-fill:\"balance\";}",
            "k{column-fill:;}",
            "l{clip-rule:evenodd;}",
        ),
    );
...
fn comments_and_priority_preserve_decoded_keyword_meaning() {
    let result = qualify(
        3261,
        concat!(
            "a{column-fill:/**/auto/**/!important;}",
            "b{column-fill:/**/balance/**/!important;}",
        ),
    );

    assert_expected(&result, &[ExpectedOutcome::Auto, ExpectedOutcome::Balance]);
...
fn css_wide_keywords_remain_profile_unsupported() {
    let result = qualify(
        3262,
        concat!(
            "a{column-fill:initial;}",
            "b{column-fill:inherit;}",
            "c{column-fill:unset;}",
            "d{column-fill:revert;}",
            "e{column-fill:revert-layer;}",
            "f{column-fill:revert-rule;}",
        ),
    );
...
fn deferred_and_whole_value_functions_fail_open_but_ordinary_functions_are_invalid() {
    let result = qualify(
        3263,
        concat!(
            "a{column-fill:var(--rule);}",
            "b{column-fill:env(rule);}",
            "c{column-fill:attr(data-rule);}",
            "d{column-fill:--rule();}",
            "e{column-fill:first-valid(auto,balance);}",
            "f{column-fill:cycle(auto,balance);}",
            "g{column-fill:interpolate(0%,0:auto,1:balance);}",
            "h{column-fill:foo();}",
            "i{column-fill:calc(1);}",
        ),
    );
...
fn function_placement_preserves_keyword_leaf_fail_open_boundary() {
    let result = qualify(
        3264,
        concat!(
            "a{column-fill:auto first-valid(balance);}",
            "b{column-fill:first-valid(auto) balance;}",
            "c{column-fill:auto foo();}",
            "d{column-fill:foo() balance;}",
            "e{column-fill:balance var(--rule);}",
            "f{column-fill:foo(var(--rule));}",
        ),
    );
...
fn element_kind_and_multicol_applicability_are_not_inputs_to_qualification() {
    ...
    assert_eq!(result.column_fill_observations()[0].occurrence_index(), 0);
    assert_eq!(result.column_fill_observations()[1].occurrence_index(), 1);
    assert_eq!(result.column_fill_observations()[2].occurrence_index(), 2);
}

#[test]
fn one_run_interleaves_column_fill_with_every_accepted_leaf() {
    let result = qualify(
        3266,
        concat!(
            "a{direction:ltr;}",
...
fn duplicate_declarations_keep_distinct_run_local_placement() {
    let result = qualify(3267, "a{column-fill:auto;}b{column-fill:auto;}");

    assert_expected(&result, &[ExpectedOutcome::Auto, ExpectedOutcome::Auto]);
    assert_eq!(result.column_fill_observations()[0].occurrence_index(), 0);
    assert_eq!(result.column_fill_observations()[1].occurrence_index(), 1);
    assert_ne!(
        result.column_fill_observations()[0]
            .placement()
            .context_id(),
        result.column_fill_observations()[1]
            .placement()
            .context_id(),
    );
}
...
fn nonordinary_declaration_shaped_contexts_are_excluded() {
    for (source_id, css) in [
        (3270, "@font-face{column-fill:balance;}"),
        (3271, "@page{column-fill:balance;}"),
        (3272, "@page{@top-left{column-fill:balance;}}"),
        (3273, "@keyframes k{from{column-fill:balance;}}"),
    ] {
        let result = qualify(source_id, css);
        assert!(
            result.column_fill_observations().is_empty(),
            "nonordinary declaration context produced a column-fill observation for {css:?}"
        );
    }
}
...
fn parser_resource_stop_preserves_committed_prefix_and_completion() {
    let result = qualify_with_limits(
        3280,
        "a{column-fill:auto;column-fill:balance;}",
...
fn repeated_and_cross_source_runs_are_semantically_deterministic() {
    ...
    assert_eq!(
        first.column_fill_observations(),
        repeated.column_fill_observations()
    );
    assert_eq!(
        first.column_fill_observations(),
        another_source.column_fill_observations()
    );
}
