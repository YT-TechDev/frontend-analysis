...
fn selector_limits() -> CssSelectorLimits {
    CssSelectorLimits::new(200_000, 128, 16 * 1024, 256 * 1024).unwrap()
}
...
fn selector_observation_resource_refusal_preserves_committed_prefix() {
    ...
    let result = analyze_css_selectors(
        ...
        tokenizer_limits(),
        parser_limits(),
        CssSelectorLimits::new(200_000, 128, 1, 256 * 1024).unwrap(),
    )
    .unwrap();
...
fn selector_depth_refusal_commits_no_partial_observation() {
    ...
    let result = analyze_css_selectors(
        ...
        tokenizer_limits(),
        parser_limits(),
        CssSelectorLimits::new(200_000, 0, 16, 256 * 1024).unwrap(),
    )
    .unwrap();
...
fn selector_algorithm_step_refusal_commits_no_partial_observation() {
    ...
    let result = analyze_css_selectors(
        ...
        tokenizer_limits(),
        parser_limits(),
        CssSelectorLimits::new(1, 128, 16, 256 * 1024).unwrap(),
    )
    .unwrap();
