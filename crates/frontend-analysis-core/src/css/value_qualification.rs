...
/// Run-local locator for the exact tokenizer item selected during
/// authoritative `transform` `translate()` direct `<length-percentage>`
/// argument recognition (#651). Mirrors `CssTransformScaleArgumentEvidenceRef`:
/// the index is evidence placement, not an interpreted numeric magnitude,
/// and always points at the exact tokenizer-owned direct `Number`,
/// `Dimension`, or `Percentage` token retained inside one `translate()`
/// argument slot, resolved through `transform_translate_argument_token`.
/// This is a distinct type from `CssTransformTranslate3dArgumentEvidenceRef`:
/// `translate()` and `translate3d()` remain distinct semantic placements
/// even though both accept a direct `<length-percentage>` shape.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformTranslateArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformTranslateArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One direct authored `translate()` transform-function argument's
/// tokenizer-owned kind (#651): `TranslateArgument := DirectLength |
/// DirectPercentage`. A `Percentage` is never collapsed into a `Length`,
/// mirroring `CssTransformTranslate3dXyArgumentKind`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformTranslateArgumentKind {
    Length,
    Percentage,
}

/// One direct authored `translate()` argument, preserving authored kind
/// and exact tokenizer-owned evidence without any interpreted-length
/// conversion (#651). `DirectLength` here is either a `Dimension` with a
/// recognized CSS length unit or a direct exact-zero `Number`, reusing the
/// accepted `translate` (#606) / `translate3d` (#647) zero-as-length
/// theorem.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformTranslateArgument {
    kind: CssTransformTranslateArgumentKind,
    evidence_ref: CssTransformTranslateArgumentEvidenceRef,
}

impl CssTransformTranslateArgument {
    pub(crate) const fn kind(&self) -> CssTransformTranslateArgumentKind {
        self.kind
    }

    pub(crate) const fn evidence_ref(&self) -> CssTransformTranslateArgumentEvidenceRef {
        self.evidence_ref
    }
}

/// One qualified `translate()` transform component's ordered argument list
/// (#651), carrying authored one-vs-two cardinality structurally so a
/// qualified component can never represent zero, three, or a synthesized
/// omitted argument, mirroring `CssTransformScaleArguments`. `translate(10px)`
/// is `One`; `translate(10px, 20px)` is `Two`; there is no third shape to
/// construct, and no second argument is ever synthesized for the `One`
/// shape.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformTranslateArguments {
    One(CssTransformTranslateArgument),
    Two(CssTransformTranslateArgument, CssTransformTranslateArgument),
}

impl CssTransformTranslateArguments {
    /// The always-present first authored argument.
    pub(crate) const fn first(&self) -> CssTransformTranslateArgument {
        match self {
            Self::One(first) | Self::Two(first, _) => *first,
        }
    }
    ...
    /// The second authored argument, or `None` when only one was authored.
    /// Never synthesized: an authored one-argument `translate()` has no
    /// second evidence to report.
    pub(crate) const fn second(&self) -> Option<CssTransformTranslateArgument> {
        match self {
            Self::One(_) => None,
            Self::Two(_, second) => Some(*second),
        }
...
/// One qualified authored `translate()` transform component: one or two
/// ordered direct authored `<length-percentage>` arguments (#651), each
/// carrying the exact tokenizer-owned evidence retained at its own
/// semantic argument slot. This leaf constructs no displacement, resolves
/// no percentage basis, and performs no unit conversion.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformTranslateFunction {
    arguments: CssTransformTranslateArguments,
}

impl CssTransformTranslateFunction {
    pub(crate) const fn arguments(&self) -> &CssTransformTranslateArguments {
        &self.arguments
    }
}
...
/// Run-local locator for the exact tokenizer item selected during
/// authoritative `transform` `translateY()` direct `<length-percentage>`
/// argument recognition (#655). Mirrors
/// `CssTransformTranslateXArgumentEvidenceRef`: the index is evidence
/// placement, not an interpreted numeric magnitude, and always points at
/// the exact tokenizer-owned direct `Number`, `Dimension`, or `Percentage`
/// token retained inside the single `translateY()` argument slot, resolved
/// through `transform_translatey_argument_token`. This is a distinct type
/// from `CssTransformTranslateXArgumentEvidenceRef`,
/// `CssTransformTranslateArgumentEvidenceRef`, and
/// `CssTransformTranslate3dArgumentEvidenceRef`: `translateY()`,
/// `translateX()`, `translate()`, and `translate3d()` remain distinct
/// semantic placements even though all four accept a direct
/// `<length-percentage>` shape.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformTranslateYArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformTranslateYArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One direct authored `translateY()` argument's tokenizer-owned kind
/// (#655): `TranslateYArgument := DirectLength | DirectPercentage`. A
/// `Percentage` is never collapsed into a `Length`, mirroring
/// `CssTransformTranslateXArgumentKind`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformTranslateYArgumentKind {
    Length,
    Percentage,
}

/// One direct authored `translateY()` argument, preserving authored kind
/// and exact tokenizer-owned evidence without any interpreted-length
/// conversion (#655). `DirectLength` here is either a `Dimension` with a
/// recognized CSS length unit or a direct exact-zero `Number`, reusing the
/// accepted `translate` (#606) / `translate3d` (#647) / `translate()`
/// (#651) / `translateX()` (#653) zero-as-length theorem.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformTranslateYArgument {
    kind: CssTransformTranslateYArgumentKind,
    evidence_ref: CssTransformTranslateYArgumentEvidenceRef,
}

impl CssTransformTranslateYArgument {
    pub(crate) const fn kind(&self) -> CssTransformTranslateYArgumentKind {
        self.kind
    }

    pub(crate) const fn evidence_ref(&self) -> CssTransformTranslateYArgumentEvidenceRef {
        self.evidence_ref
    }
}

/// One qualified authored `translateY()` transform component: exactly one
/// direct authored `<length-percentage>` argument (#655), carrying the
/// exact tokenizer-owned evidence retained at its own semantic argument
/// slot. Like `translateX()` and unlike `translate()`, `translateY()` has
/// no optional second argument, so this leaf carries a single argument
/// directly rather than an ordered one-or-two cardinality. This leaf
/// constructs no displacement, resolves no percentage basis, and performs
/// no unit conversion.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformTranslateYFunction {
    argument: CssTransformTranslateYArgument,
}

impl CssTransformTranslateYFunction {
    pub(crate) const fn argument(&self) -> CssTransformTranslateYArgument {
        self.argument
    }
}

/// Run-local locator for the exact tokenizer item selected during
/// authoritative `transform` `translateZ()` direct `<length>` argument
/// recognition (#657). Mirrors `CssTransformTranslateYArgumentEvidenceRef`:
/// the index is evidence placement, not an interpreted numeric magnitude,
/// and always points at the exact tokenizer-owned direct `Number` or
/// `Dimension` token retained inside the single `translateZ()` argument
/// slot, resolved through `transform_translatez_argument_token`. This is a
/// distinct type from `CssTransformTranslateXArgumentEvidenceRef`,
/// `CssTransformTranslateYArgumentEvidenceRef`,
/// `CssTransformTranslateArgumentEvidenceRef`, and
/// `CssTransformTranslate3dArgumentEvidenceRef`: `translateZ()`,
/// `translateY()`, `translateX()`, `translate()`, and `translate3d()`
/// remain distinct semantic placements even though `translateZ()` shares
/// its direct `<length>`-only scalar theorem with the `translate3d()` Z
/// slot.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformTranslateZArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformTranslateZArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One qualified authored `translateZ()` transform component: exactly one
/// direct authored `<length>` argument (#657), carrying the exact
/// tokenizer-owned evidence retained at its own semantic argument slot.
/// Unlike `translateX()`/`translateY()`, `translateZ()` accepts only a
/// direct `<length>`, never a `<percentage>`, so this leaf carries a bare
/// `CssTransformTranslateZArgumentEvidenceRef` directly rather than a
/// Length/Percentage `kind` distinction -- there is no Percentage role for
/// a qualified `translateZ()` argument to carry, mirroring how
/// `CssTransformTranslate3dFunction`'s Z field carries its evidence
/// directly. This leaf constructs no displacement, resolves no percentage
/// basis, and performs no unit conversion.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformTranslateZFunction {
    argument: CssTransformTranslateZArgumentEvidenceRef,
}

impl CssTransformTranslateZFunction {
    pub(crate) const fn argument(&self) -> CssTransformTranslateZArgumentEvidenceRef {
        self.argument
    }
}

/// Run-local locator for the exact tokenizer item selected during
/// authoritative `transform` `scaleX()` direct `<number>`/`<percentage>`
/// argument recognition (#659). Mirrors `CssTransformScaleArgumentEvidenceRef`:
/// the index is evidence placement, not an interpreted numeric magnitude,
/// and always points at the exact tokenizer-owned direct `Number` or
/// `Percentage` token retained inside the single `scaleX()` argument slot,
/// resolved through `transform_scalex_argument_token`. This is a distinct
/// type from `CssTransformScaleArgumentEvidenceRef`: `scaleX()` and
/// `scale()` remain distinct semantic placements even though both accept
/// the same direct `<number> | <percentage>` scalar theorem.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleXArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformScaleXArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One direct authored `scaleX()` argument's tokenizer-owned kind (#659):
/// `ScaleXArgument := DirectNumber | DirectPercentage`, reusing the
/// accepted `scale()` (#645) Number/Percentage theorem under current CSS
/// Transforms Level 2 authority. A direct `<number>` and a direct
/// `<percentage>` remain distinct authored branches; this leaf never
/// normalizes a Percentage token into an interpreted fraction or into a
/// Number token, so `scaleX(0)` and `scaleX(0%)` stay pairwise
/// distinguishable. This is a distinct type from
/// `CssTransformScaleArgumentKind`: `scaleX()` transform-function argument
/// placement and `scale()` transform-function argument placement remain
/// distinct semantic roles even though they share the same direct scalar
/// membership.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformScaleXArgumentKind {
    Number,
    Percentage,
}

/// One qualified authored `scaleX()` argument, preserving authored kind and
/// exact tokenizer-owned evidence without any interpreted scale-factor
/// conversion (#659).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleXArgument {
    kind: CssTransformScaleXArgumentKind,
    evidence_ref: CssTransformScaleXArgumentEvidenceRef,
}

impl CssTransformScaleXArgument {
    pub(crate) const fn kind(&self) -> CssTransformScaleXArgumentKind {
        self.kind
    }

    pub(crate) const fn evidence_ref(&self) -> CssTransformScaleXArgumentEvidenceRef {
        self.evidence_ref
    }
}

/// One qualified authored `scaleX()` transform component: exactly one
/// direct authored `<number> | <percentage>` argument (#659), carrying the
/// exact tokenizer-owned evidence retained at its own semantic argument
/// slot. Unlike `scale()`, `scaleX()` has no optional second argument, so
/// this leaf carries a single argument directly rather than an ordered
/// one-or-two cardinality, mirroring how `CssTransformTranslateXFunction`
/// carries a single argument distinct from `CssTransformTranslateFunction`'s
/// one-or-two cardinality. This leaf constructs no scale factor, performs
/// no matrix construction, and resolves no computed transform.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleXFunction {
    argument: CssTransformScaleXArgument,
}

impl CssTransformScaleXFunction {
    pub(crate) const fn argument(&self) -> CssTransformScaleXArgument {
        self.argument
    }
}

/// One qualified authored `scaleY()` argument's run-local reference to its
/// exact tokenizer-owned `Number` or `Percentage` token, resolved through
/// `transform_scaley_argument_token`. This is a distinct type from
/// `CssTransformScaleArgumentEvidenceRef` and
/// `CssTransformScaleXArgumentEvidenceRef`: `scaleY()`, `scale()`, and
/// `scaleX()` remain distinct semantic placements even though all three
/// accept the same direct `<number> | <percentage>` scalar theorem (#661).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleYArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformScaleYArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One direct authored `scaleY()` argument's tokenizer-owned kind (#661):
/// `ScaleYArgument := DirectNumber | DirectPercentage`, reusing the accepted
/// `scale()` (#645) / `scaleX()` (#659) Number/Percentage theorem under
/// current CSS Transforms Level 2 authority. A direct `<number>` and a
/// direct `<percentage>` remain distinct authored branches; this leaf never
/// normalizes a Percentage token into an interpreted fraction or into a
/// Number token, so `scaleY(0)` and `scaleY(0%)` stay pairwise
/// distinguishable. This is a distinct type from
/// `CssTransformScaleArgumentKind` and `CssTransformScaleXArgumentKind`:
/// `scaleY()` transform-function argument placement remains a distinct
/// semantic role from `scale()` and `scaleX()` even though they share the
/// same direct scalar membership.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformScaleYArgumentKind {
    Number,
    Percentage,
}

/// One qualified authored `scaleY()` argument, preserving authored kind and
/// exact tokenizer-owned evidence without any interpreted scale-factor
/// conversion (#661).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleYArgument {
    kind: CssTransformScaleYArgumentKind,
    evidence_ref: CssTransformScaleYArgumentEvidenceRef,
}

impl CssTransformScaleYArgument {
    pub(crate) const fn kind(&self) -> CssTransformScaleYArgumentKind {
        self.kind
    }

    pub(crate) const fn evidence_ref(&self) -> CssTransformScaleYArgumentEvidenceRef {
        self.evidence_ref
    }
}

/// One qualified authored `scaleY()` transform component: exactly one
/// direct authored `<number> | <percentage>` argument (#661), carrying the
/// exact tokenizer-owned evidence retained at its own semantic argument
/// slot. Mirrors `CssTransformScaleXFunction`'s single-argument shape
/// exactly, while remaining a distinct type: `scaleY()` never shares
/// representation with `scaleX()` or `scale()`. This leaf constructs no
/// scale factor, performs no matrix construction, and resolves no computed
/// transform.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleYFunction {
    argument: CssTransformScaleYArgument,
}

impl CssTransformScaleYFunction {
    pub(crate) const fn argument(&self) -> CssTransformScaleYArgument {
        self.argument
    }
}

/// One qualified authored `scaleZ()` argument's run-local reference to its
/// exact tokenizer-owned `Number` or `Percentage` token, resolved through
/// `transform_scalez_argument_token`. This is a distinct type from
/// `CssTransformScaleArgumentEvidenceRef`, `CssTransformScaleXArgumentEvidenceRef`,
/// and `CssTransformScaleYArgumentEvidenceRef`: `scaleZ()`, `scaleY()`,
/// `scaleX()`, and `scale()` remain distinct semantic placements even
/// though all four accept the same direct `<number> | <percentage>` scalar
/// theorem (#663). `scaleZ()` also shares its `Z` suffix with
/// `translateZ()`, but `translateZ()`'s argument grammar is `<length>`, not
/// `<number> | <percentage>`, so this evidence type is never shared with
/// `CssTransformTranslateZArgumentEvidenceRef` either.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleZArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformScaleZArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One direct authored `scaleZ()` argument's tokenizer-owned kind (#663):
/// `ScaleZArgument := DirectNumber | DirectPercentage`, reusing the accepted
/// `scale()` (#645) / `scaleX()` (#659) / `scaleY()` (#661) Number/Percentage
/// theorem under current CSS Transforms Level 2 authority. A direct
/// `<number>` and a direct `<percentage>` remain distinct authored branches;
/// this leaf never normalizes a Percentage token into an interpreted
/// fraction or into a Number token, so `scaleZ(0)` and `scaleZ(0%)` stay
/// pairwise distinguishable. This is a distinct type from
/// `CssTransformScaleArgumentKind`, `CssTransformScaleXArgumentKind`, and
/// `CssTransformScaleYArgumentKind`: `scaleZ()` transform-function argument
/// placement remains a distinct semantic role from `scale()`, `scaleX()`,
/// and `scaleY()` even though they share the same direct scalar membership.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformScaleZArgumentKind {
    Number,
    Percentage,
}

/// One qualified authored `scaleZ()` argument, preserving authored kind and
/// exact tokenizer-owned evidence without any interpreted scale-factor
/// conversion (#663).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleZArgument {
    kind: CssTransformScaleZArgumentKind,
    evidence_ref: CssTransformScaleZArgumentEvidenceRef,
}

impl CssTransformScaleZArgument {
    pub(crate) const fn kind(&self) -> CssTransformScaleZArgumentKind {
        self.kind
    }

    pub(crate) const fn evidence_ref(&self) -> CssTransformScaleZArgumentEvidenceRef {
        self.evidence_ref
    }
}

/// One qualified authored `scaleZ()` transform component: exactly one
/// direct authored `<number> | <percentage>` argument (#663), carrying the
/// exact tokenizer-owned evidence retained at its own semantic argument
/// slot. Mirrors `CssTransformScaleXFunction`/`CssTransformScaleYFunction`'s
/// single-argument shape exactly, while remaining a distinct type: `scaleZ()`
/// never shares representation with `scaleX()`, `scaleY()`, `scale()`, or
/// `translateZ()`. This leaf constructs no scale factor, performs no matrix
/// construction, and resolves no computed transform.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformScaleZFunction {
    argument: CssTransformScaleZArgument,
}

impl CssTransformScaleZFunction {
    pub(crate) const fn argument(&self) -> CssTransformScaleZArgument {
        self.argument
    }
}
...
/// One qualified `transform` `rotate()` argument's run-local evidence
/// reference, resolved through `transform_rotate_argument_token` (#667).
/// This is a distinct type from `CssTransformRotate3dArgumentEvidenceRef`:
/// `rotate()`'s single authored slot and `rotate3d()`'s positional
/// fourth-slot angle argument remain distinct semantic placements even
/// though both accept the same direct `<angle> | <zero>` scalar theorem.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformRotateArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One direct authored `rotate()` argument's tokenizer-owned kind (#667):
/// `RotateArgument := DirectAngle | DirectZero`, reusing the accepted
/// `rotate3d()` (#649) fourth-slot `<angle> | <zero>` theorem under current
/// CSS Transforms Level 2 / CSS Values authority. A direct `<angle>` and a
/// direct literal `<zero>` remain distinct authored branches: this leaf
/// never collapses `rotate(0)` into the `Angle` branch, never collapses
/// `rotate(0deg)` into the `Zero` branch merely because its magnitude is
/// zero, and never widens the general `<angle>` grammar to accept a
/// unitless Number. This is a distinct type from
/// `CssTransformRotate3dAngleArgument`: `rotate()` transform-function
/// argument placement and `rotate3d()`'s fourth-slot placement remain
/// distinct semantic roles even though they share the same direct scalar
/// membership.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformRotateArgumentKind {
    Angle,
    Zero,
}

/// One qualified authored `rotate()` argument, preserving authored kind and
/// exact tokenizer-owned evidence without any angle normalization or
/// machine-number conversion (#667).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateArgument {
    kind: CssTransformRotateArgumentKind,
    evidence_ref: CssTransformRotateArgumentEvidenceRef,
}

impl CssTransformRotateArgument {
    pub(crate) const fn kind(&self) -> CssTransformRotateArgumentKind {
        self.kind
    }

    pub(crate) const fn evidence_ref(&self) -> CssTransformRotateArgumentEvidenceRef {
        self.evidence_ref
    }
}

/// One qualified authored `rotate()` transform component: exactly one
/// direct authored `<angle> | <zero>` argument (#667), carrying the exact
/// tokenizer-owned evidence retained at its own semantic argument slot.
/// `rotate()` never synthesizes an axis, never normalizes into
/// `rotate3d()`, and never shares representation with the accepted
/// longhand `rotate` property qualifier (#604) or a future `rotateX()`/
/// `rotateY()`/`rotateZ()` leaf, even though all of them can carry angle
/// evidence: repeated grammar shape alone is never abstraction authority.
/// This leaf constructs no rotation matrix and resolves no computed
/// transform.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateFunction {
    argument: CssTransformRotateArgument,
}

impl CssTransformRotateFunction {
    pub(crate) const fn argument(&self) -> CssTransformRotateArgument {
        self.argument
    }
}

/// One qualified `transform` `rotateX()` argument's run-local evidence
/// reference, resolved through `transform_rotatex_argument_token` (#669).
/// This is a distinct type from `CssTransformRotateArgumentEvidenceRef` and
/// `CssTransformRotate3dArgumentEvidenceRef`: `rotateX()`'s single authored
/// slot remains a distinct semantic placement from `rotate()`'s single
/// authored slot and from `rotate3d()`'s positional fourth-slot angle
/// argument even though all three accept the same direct
/// `<angle> | <zero>` scalar theorem.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateXArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformRotateXArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One direct authored `rotateX()` argument's tokenizer-owned kind (#669):
/// `RotateXArgument := DirectAngle | DirectZero`, reusing the accepted
/// rotate-family (#667) `<angle> | <zero>` theorem under current CSS
/// Transforms Level 2 / CSS Values authority. A direct `<angle>` and a
/// direct literal `<zero>` remain distinct authored branches: this leaf
/// never collapses `rotateX(0)` into the `Angle` branch, never collapses
/// `rotateX(0deg)` into the `Zero` branch merely because its magnitude is
/// zero, and never widens the general `<angle>` grammar to accept a
/// unitless Number. This is a distinct type from
/// `CssTransformRotateArgumentKind` and `CssTransformRotate3dAngleArgument`:
/// `rotateX()` transform-function argument placement remains a distinct
/// semantic role from `rotate()`'s and `rotate3d()`'s fourth-slot placement
/// even though they share the same direct scalar membership.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformRotateXArgumentKind {
    Angle,
    Zero,
}

/// One qualified authored `rotateX()` argument, preserving authored kind and
/// exact tokenizer-owned evidence without any angle normalization or
/// machine-number conversion (#669).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateXArgument {
    kind: CssTransformRotateXArgumentKind,
    evidence_ref: CssTransformRotateXArgumentEvidenceRef,
}

impl CssTransformRotateXArgument {
    pub(crate) const fn kind(&self) -> CssTransformRotateXArgumentKind {
        self.kind
    }

    pub(crate) const fn evidence_ref(&self) -> CssTransformRotateXArgumentEvidenceRef {
        self.evidence_ref
    }
}

/// One qualified authored `rotateX()` transform component: exactly one
/// direct authored `<angle> | <zero>` argument (#669), carrying the exact
/// tokenizer-owned evidence retained at its own semantic argument slot.
/// `rotateX()` never synthesizes an axis, never normalizes into `rotate()`
/// or `rotate3d()`, and never shares representation with the accepted
/// longhand `rotate` property qualifier (#604) or a future `rotateY()`/
/// `rotateZ()` leaf, even though all of them can carry angle evidence:
/// repeated grammar shape alone is never abstraction authority. This leaf
/// constructs no rotation matrix and resolves no computed transform.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateXFunction {
    argument: CssTransformRotateXArgument,
}

impl CssTransformRotateXFunction {
    pub(crate) const fn argument(&self) -> CssTransformRotateXArgument {
        self.argument
    }
}

/// One qualified `transform` `rotateY()` argument's run-local evidence
/// reference, resolved through `transform_rotatey_argument_token` (#671).
/// This is a distinct type from `CssTransformRotateArgumentEvidenceRef`,
/// `CssTransformRotateXArgumentEvidenceRef`, and
/// `CssTransformRotate3dArgumentEvidenceRef`: `rotateY()`'s single authored
/// slot remains a distinct semantic placement from `rotate()`'s single
/// authored slot, from `rotateX()`'s single authored slot, and from
/// `rotate3d()`'s positional fourth-slot angle argument even though all four
/// accept the same direct `<angle> | <zero>` scalar theorem.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateYArgumentEvidenceRef {
    lexical_item_index: usize,
}

impl CssTransformRotateYArgumentEvidenceRef {
    pub(crate) const fn lexical_item_index(&self) -> usize {
        self.lexical_item_index
    }
}

/// One direct authored `rotateY()` argument's tokenizer-owned kind (#671):
/// `RotateYArgument := DirectAngle | DirectZero`, reusing the accepted
/// rotate-family (#667) `<angle> | <zero>` theorem under current CSS
/// Transforms Level 2 / CSS Values authority. A direct `<angle>` and a
/// direct literal `<zero>` remain distinct authored branches: this leaf
/// never collapses `rotateY(0)` into the `Angle` branch, never collapses
/// `rotateY(0deg)` into the `Zero` branch merely because its magnitude is
/// zero, and never widens the general `<angle>` grammar to accept a
/// unitless Number. This is a distinct type from
/// `CssTransformRotateArgumentKind`, `CssTransformRotateXArgumentKind`, and
/// `CssTransformRotate3dAngleArgument`: `rotateY()` transform-function
/// argument placement remains a distinct semantic role from `rotate()`'s,
/// `rotateX()`'s, and `rotate3d()`'s fourth-slot placement even though they
/// share the same direct scalar membership.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CssTransformRotateYArgumentKind {
    Angle,
    Zero,
}

/// One qualified authored `rotateY()` argument, preserving authored kind and
/// exact tokenizer-owned evidence without any angle normalization or
/// machine-number conversion (#671).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateYArgument {
    kind: CssTransformRotateYArgumentKind,
    evidence_ref: CssTransformRotateYArgumentEvidenceRef,
}

impl CssTransformRotateYArgument {
    pub(crate) const fn kind(&self) -> CssTransformRotateYArgumentKind {
        self.kind
    }

    pub(crate) const fn evidence_ref(&self) -> CssTransformRotateYArgumentEvidenceRef {
        self.evidence_ref
    }
}

/// One qualified authored `rotateY()` transform component: exactly one
/// direct authored `<angle> | <zero>` argument (#671), carrying the exact
/// tokenizer-owned evidence retained at its own semantic argument slot.
/// `rotateY()` never synthesizes an axis, never normalizes into `rotate()`,
/// `rotateX()`, or `rotate3d()`, and never shares representation with the
/// accepted longhand `rotate` property qualifier (#604) or a future
/// `rotateZ()` leaf, even though all of them can carry angle evidence:
/// repeated grammar shape alone is never abstraction authority. This leaf
/// constructs no rotation matrix and resolves no computed transform.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct CssTransformRotateYFunction {
    argument: CssTransformRotateYArgument,
}

impl CssTransformRotateYFunction {
    pub(crate) const fn argument(&self) -> CssTransformRotateYArgument {
        self.argument
    }
}
...
pub(crate) enum CssTransformFunction {
    ...
    TranslateY(CssTransformTranslateYFunction),
    TranslateZ(CssTransformTranslateZFunction),
...
/// <length-percentage>?) | translateX(<length-percentage>) |
...
/// rotate([<angle> | <zero>]) | rotateX([<angle> | <zero>]) ]+` (#418 /
/// #645 / #647 / #649 / #651 / #653 / #655 / #657 / #659 / #661 / #663 /
/// #665 / #667 / #669): either the dedicated whole-value `none` sentinel or
/// an ordered, possibly repeated, one-or-more list of qualified
/// `matrix()`/`scale()`/`translate3d()`/`rotate3d()`/`translate()`/
/// `translateX()`/`translateY()`/`translateZ()`/`scaleX()`/`scaleY()`/
/// `scaleZ()`/`scale3d()`/`rotate()`/`rotateX()` components in exact
...
/// `rotateX()` branches of `<transform-function>`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// <length-percentage>, <length>)`, `rotate3d() = rotate3d(<number>,
/// <number>, <number>, [<angle> | <zero>])`, and `translate() =
/// translate(<length-percentage>, <length-percentage>?)`. This observation
/// deliberately selects only the `matrix()`, `scale()`, `translate3d()`,
/// `rotate3d()`, and `translate()` branches: every other
/// `<transform-function>` stays outside selected-profile coverage rather
/// than being decided here, because deciding it would require the full
/// `<transform-function>` dispatch and the length/angle semantics this
/// leaf does not own.
///
...
/// Percentage`, then `Length` only -- qualify a `translate3d()`, exactly
/// four position-sensitive slots -- three direct `Number` axis slots, then
/// a fourth direct `Angle | Zero` slot -- qualify a `rotate3d()`, with no
/// axis-vector normalization or all-zero-vector rejection, and one or two
/// slots each holding exactly one direct retained `Length | Percentage`
/// token qualify a `translate()`, with no synthesized omitted second
/// argument.
///
...
impl CssValueQualificationRunResult {
    ...
    /// Resolves one qualified `transform` `translate()` argument's
    /// run-local evidence reference to its exact retained tokenizer token
    /// kind, preserving authored sign spelling, integer/fraction digits,
    /// exponent spelling, and unit identity without any machine-number
    /// conversion or percentage/unit normalization. The retained token at
    ...
    /// argument's `CssTransformTranslateArgumentKind`.
    ...
    /// Resolves one qualified `transform` `translateY()` argument's
    /// run-local evidence reference to its exact retained tokenizer token
    /// kind, preserving authored sign spelling, integer/fraction digits,
    /// exponent spelling, and unit identity without any machine-number
    /// conversion or percentage/unit normalization. The retained token at
    /// the evidence position is always a direct exact-zero `Number`, a
    /// recognized-length `Dimension`, or a `Percentage`, matching the
    /// argument's `CssTransformTranslateYArgumentKind`.
    pub(crate) fn transform_translatey_argument_token(
        &self,
        evidence: CssTransformTranslateYArgumentEvidenceRef,
    ) -> Option<&CssTokenKind> {
        let item = self
            .upstream_parser_result
            .upstream_tokenizer_result()
    ...
    /// Resolves one qualified `transform` `translateZ()` argument's
    /// run-local evidence reference to its exact retained tokenizer token
    /// kind, preserving authored sign spelling, integer/fraction digits,
    /// exponent spelling, and unit identity without any machine-number
    /// conversion or unit normalization. The retained token at the evidence
    /// position is always a direct exact-zero `Number` or a
    /// recognized-length `Dimension`, never a `Percentage`: a direct
    /// `translateZ()` `Percentage` is rejected by
    /// `classify_transform_translatez_argument` before a
    /// `CssTransformTranslateZFunction` is ever constructed.
    pub(crate) fn transform_translatez_argument_token(
        &self,
        evidence: CssTransformTranslateZArgumentEvidenceRef,
    ) -> Option<&CssTokenKind> {
        let item = self
            .upstream_parser_result
            .upstream_tokenizer_result()
    ...
    /// Resolves one qualified `transform` `scaleX()` argument's run-local
    /// evidence reference to its exact retained tokenizer token kind,
    /// preserving authored sign spelling, integer/fraction digits, and
    /// exponent spelling without any machine-number conversion. The
    /// retained token at the evidence position is always a direct `Number`
    /// or `Percentage`, matching the argument's
    /// `CssTransformScaleXArgumentKind`.
    pub(crate) fn transform_scalex_argument_token(
        &self,
        evidence: CssTransformScaleXArgumentEvidenceRef,
    ) -> Option<&CssTokenKind> {
        let item = self
            .upstream_parser_result
            .upstream_tokenizer_result()
    ...
    /// Resolves one qualified `transform` `scaleY()` argument's run-local
    /// evidence reference to its exact retained tokenizer token kind,
    /// preserving authored sign spelling, integer/fraction digits, and
    /// exponent spelling without any machine-number conversion. The
    /// retained token at the evidence position is always a direct `Number`
    /// or `Percentage`, matching the argument's
    /// `CssTransformScaleYArgumentKind`.
    pub(crate) fn transform_scaley_argument_token(
        &self,
        evidence: CssTransformScaleYArgumentEvidenceRef,
    ) -> Option<&CssTokenKind> {
        let item = self
            .upstream_parser_result
            .upstream_tokenizer_result()
    ...
    /// Resolves one qualified `transform` `scaleZ()` argument's run-local
    /// evidence reference to its exact retained tokenizer token kind,
    /// preserving authored sign spelling, integer/fraction digits, and
    /// exponent spelling without any machine-number conversion. The
    /// retained token at the evidence position is always a direct `Number`
    /// or `Percentage`, matching the argument's
    /// `CssTransformScaleZArgumentKind`.
    ...
    /// Resolves one qualified `transform` `rotate()` argument's run-local
    /// evidence reference to its exact retained tokenizer token kind,
    ...
    /// `CssTransformRotateArgumentKind` is `Angle`, or an exact-zero `Number`
    /// when it is `Zero`, mirroring `transform_rotate3d_argument_token`'s
    /// fourth-slot resolution while remaining a distinct accessor for a
    /// distinct evidence type (#667).
    pub(crate) fn transform_rotate_argument_token(
        &self,
        evidence: CssTransformRotateArgumentEvidenceRef,
    ) -> Option<&CssTokenKind> {
        let item = self
            .upstream_parser_result
            .upstream_tokenizer_result()
    ...
    /// Resolves one qualified `transform` `rotateX()` argument's run-local
    /// evidence reference to its exact retained tokenizer token kind,
    /// preserving authored sign spelling, integer/fraction digits, exponent
    /// spelling, and unit identity without any machine-number conversion or
    /// angle normalization. The retained token at the evidence position is a
    /// recognized-angle `Dimension` when the argument's
    /// `CssTransformRotateXArgumentKind` is `Angle`, or an exact-zero
    /// `Number` when it is `Zero`, mirroring `transform_rotate_argument_token`
    /// while remaining a distinct accessor for a distinct evidence type
    /// (#669).
    pub(crate) fn transform_rotatex_argument_token(
        &self,
        evidence: CssTransformRotateXArgumentEvidenceRef,
    ) -> Option<&CssTokenKind> {
        let item = self
            .upstream_parser_result
            .upstream_tokenizer_result()
    ...
    /// Resolves one qualified `transform` `rotateY()` argument's run-local
    /// evidence reference to its exact retained tokenizer token kind,
    /// preserving authored sign spelling, integer/fraction digits, exponent
    /// spelling, and unit identity without any machine-number conversion or
    /// angle normalization. The retained token at the evidence position is a
    /// recognized-angle `Dimension` when the argument's
    /// `CssTransformRotateYArgumentKind` is `Angle`, or an exact-zero
    /// `Number` when it is `Zero`, mirroring `transform_rotatex_argument_token`
    /// while remaining a distinct accessor for a distinct evidence type
    /// (#671).
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformTranslateArgumentClass {
    Length(CssTransformTranslateArgumentEvidenceRef),
    Percentage(CssTransformTranslateArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `translate()` argument slot against
/// the selected profile's single accepted shape -- a direct exact-zero
/// `Number`, a `Dimension` with a recognized CSS length unit, or a
/// `Percentage` (#651), mirroring `classify_transform_translate3d_argument`'s
/// X/Y shape exactly, reusing its exact-zero and recognized-length-unit
/// theorem. This is a distinct classification function from both
/// `classify_transform_translate3d_argument` and the accepted longhand
/// `classify_translate_component`: `translate()` argument placement,
/// `translate3d()` argument placement, and standalone `translate` property
/// placement remain distinct semantic roles even though they share the
/// same direct `<length-percentage>` scalar theorem.
///
/// Whitespace and Comment trivia are excluded exactly as for `matrix()`/
/// `scale()`/`translate3d()` arguments. An authored-empty slot has no
/// retained semantic token and is decisively `Invalid`. A slot headed by a
/// `Function` token is `OpaqueFunction` -- a structurally feasible
/// position whose validity depends on calculated-value semantics this leaf
/// does not own -- only when the slot is exactly one complete Function
/// extent; a recognized generic whole-value-only Function name occupying
/// this non-whole-value position is decisively `Invalid` instead,
...
/// is decisively `Invalid`, as is any slot carrying more than one retained
/// semantic token.
fn classify_transform_translate_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformTranslateArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformTranslateArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => {
                CssTransformTranslateArgumentClass::Invalid
            }
            Some(_) => CssTransformTranslateArgumentClass::OpaqueFunction,
            None => CssTransformTranslateArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Number { value, .. } if is_direct_zero_numeric_value(value) => {
            CssTransformTranslateArgumentClass::Length(CssTransformTranslateArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Dimension { unit, .. } if is_css_length_unit(unit) => {
        ...
        CssTokenKind::Percentage { .. } => CssTransformTranslateArgumentClass::Percentage(
            CssTransformTranslateArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            },
        ),
        _ => CssTransformTranslateArgumentClass::Invalid,
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformTranslateYArgumentClass {
    Length(CssTransformTranslateYArgumentEvidenceRef),
    Percentage(CssTransformTranslateYArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `translateY()` argument slot against
/// the selected profile's single accepted shape -- a direct exact-zero
/// `Number`, a `Dimension` with a recognized CSS length unit, or a
/// `Percentage` (#655), mirroring `classify_transform_translatex_argument`
/// exactly, reusing its exact-zero and recognized-length-unit theorem.
/// This is a distinct classification function from
/// `classify_transform_translatex_argument`,
/// `classify_transform_translate_argument`, and
/// `classify_transform_translate3d_argument`: `translateY()` argument
/// placement, `translateX()` argument placement, `translate()` argument
/// placement, and `translate3d()` argument placement remain distinct
/// semantic roles even though they share the same direct
/// `<length-percentage>` scalar theorem.
///
/// Whitespace and Comment trivia are excluded exactly as for `matrix()`/
/// `scale()`/`translate3d()`/`translate()`/`translateX()` arguments. An
/// authored-empty slot has no retained semantic token and is decisively
/// `Invalid`. A slot headed by a `Function` token is `OpaqueFunction` -- a
/// structurally feasible position whose validity depends on
/// calculated-value semantics this leaf does not own -- only when the slot
/// is exactly one complete Function extent; a recognized generic
/// whole-value-only Function name occupying this non-whole-value position
/// is decisively `Invalid` instead, mirroring the accepted
/// `matrix`/`scale`/`translate3d`/`translate`/`translateX` boundary. A
/// Function followed by further retained material in the same slot is
/// directly visible structural failure and stays decisively `Invalid`.
///
/// A non-zero unitless `Number` and a `Dimension` with an unrecognized unit
...
/// is decisively `Invalid`, as is any slot carrying more than one retained
/// semantic token.
fn classify_transform_translatey_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformTranslateYArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformTranslateYArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => {
                CssTransformTranslateYArgumentClass::Invalid
            }
            Some(_) => CssTransformTranslateYArgumentClass::OpaqueFunction,
            None => CssTransformTranslateYArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Number { value, .. } if is_direct_zero_numeric_value(value) => {
            CssTransformTranslateYArgumentClass::Length(CssTransformTranslateYArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Dimension { unit, .. } if is_css_length_unit(unit) => {
        ...
        CssTokenKind::Percentage { .. } => CssTransformTranslateYArgumentClass::Percentage(
            CssTransformTranslateYArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            },
        ),
        _ => CssTransformTranslateYArgumentClass::Invalid,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformTranslateZArgumentClass {
    Length(CssTransformTranslateZArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `translateZ()` argument slot against
/// the selected profile's single accepted shape -- a direct exact-zero
/// `Number` or a `Dimension` with a recognized CSS length unit (#657),
/// reusing `classify_transform_translatey_argument`'s exact-zero and
/// recognized-length-unit theorem. Unlike
/// `classify_transform_translatex_argument`/
/// `classify_transform_translatey_argument`, a direct `Percentage` is never
/// a member here: `translateZ()` accepts only `<length>`, so this
/// classifier has no `Percentage` class to produce -- a direct `Percentage`
/// token falls straight through to `Invalid`, including `0%`, which is
/// never treated as zero `Length` merely because its numeric magnitude is
/// zero. This is a distinct classification function from
/// `classify_transform_translatex_argument`,
/// `classify_transform_translatey_argument`,
/// `classify_transform_translate_argument`, and
/// `classify_transform_translate3d_argument`: `translateZ()` argument
/// placement remains a distinct semantic role even though its direct
/// `<length>` scalar theorem is shared with the `translate3d()` Z slot.
///
/// Whitespace and Comment trivia are excluded exactly as for `matrix()`/
/// `scale()`/`translate3d()`/`translate()`/`translateX()`/`translateY()`
...
/// generic whole-value-only Function name occupying this non-whole-value
/// position is decisively `Invalid` instead, mirroring the accepted
/// `matrix`/`scale`/`translate3d`/`translate`/`translateX`/`translateY`
/// boundary. A Function followed by further retained material in the same
/// slot is directly visible structural failure and stays decisively
/// `Invalid`.
///
/// A non-zero unitless `Number`, a direct `Percentage`, and a `Dimension`
/// with an unrecognized unit (e.g. `deg`) all fall through to `Invalid`:
/// this leaf never interprets an arbitrary `Number` as `Length` and never
/// treats a `Percentage` as `<length>`, and no range restriction or
/// machine-number conversion is applied, so exact authored evidence stays
/// authoritative for membership. A `Dimension`, `Ident`, or `String` token
/// that is not a recognized length is a direct token-category failure and
/// is decisively `Invalid`, as is any slot carrying more than one retained
/// semantic token.
fn classify_transform_translatez_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformTranslateZArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformTranslateZArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => {
                CssTransformTranslateZArgumentClass::Invalid
            }
            Some(_) => CssTransformTranslateZArgumentClass::OpaqueFunction,
            None => CssTransformTranslateZArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Number { value, .. } if is_direct_zero_numeric_value(value) => {
            CssTransformTranslateZArgumentClass::Length(CssTransformTranslateZArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Dimension { unit, .. } if is_css_length_unit(unit) => {
        ...
        _ => CssTransformTranslateZArgumentClass::Invalid,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformScaleXArgumentClass {
    Number(CssTransformScaleXArgumentEvidenceRef),
    Percentage(CssTransformScaleXArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `scaleX()` argument slot against the
/// selected profile's single accepted shape -- a direct retained `<number>`
/// or a direct retained `<percentage>` (#659), mirroring
/// `classify_transform_scale_argument` exactly except for the distinct
/// `CssTransformScaleXArgumentEvidenceRef` evidence type: `scaleX()`
/// argument placement and `scale()` argument placement remain distinct
/// semantic roles even though they share the same direct scalar theorem
/// under current CSS Transforms Level 2 authority.
///
/// Whitespace and Comment trivia are excluded exactly as for `scale()`
/// arguments. An authored-empty slot has no retained semantic token and is
/// decisively `Invalid`. A slot headed by a `Function` token is
/// `OpaqueFunction` -- a structurally feasible position whose validity
/// depends on calculated-value semantics this leaf does not own -- only
...
/// generic whole-value-only Function name occupying this non-whole-value
/// position is decisively `Invalid` instead, mirroring the accepted
/// `scale` boundary. A Function followed by further retained material in
/// the same slot is directly visible structural failure and stays
/// decisively `Invalid`.
///
/// A direct `Number` or `Percentage` qualifies regardless of
/// `CssNumberType`; no range restriction and no machine-number conversion
/// is applied, so exact authored evidence stays authoritative for
/// membership and a `Percentage` is never collapsed into a `Number` -- this
/// retains current CSS Transforms Level 2 Percentage support rather than
/// regressing to the older Level 1 `<number>`-only grammar. A `Dimension`,
/// `Ident`, or `String` token is a direct token-category failure and is
/// decisively `Invalid`, as is any slot carrying more than one retained
/// semantic token.
fn classify_transform_scalex_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformScaleXArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformScaleXArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => CssTransformScaleXArgumentClass::Invalid,
            Some(_) => CssTransformScaleXArgumentClass::OpaqueFunction,
            None => CssTransformScaleXArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Number { .. } => {
            CssTransformScaleXArgumentClass::Number(CssTransformScaleXArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Percentage { .. } => {
            CssTransformScaleXArgumentClass::Percentage(CssTransformScaleXArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        _ => CssTransformScaleXArgumentClass::Invalid,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformScaleYArgumentClass {
    Number(CssTransformScaleYArgumentEvidenceRef),
    Percentage(CssTransformScaleYArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `scaleY()` argument slot against the
/// selected profile's single accepted shape -- a direct retained `<number>`
/// or a direct retained `<percentage>` (#661), mirroring
/// `classify_transform_scalex_argument` exactly except for the distinct
/// `CssTransformScaleYArgumentEvidenceRef` evidence type: `scaleY()`
/// argument placement, `scaleX()` argument placement, and `scale()`
/// argument placement remain distinct semantic roles even though they share
/// the same direct scalar theorem under current CSS Transforms Level 2
/// authority.
///
/// Whitespace and Comment trivia are excluded exactly as for `scaleX()`
/// arguments. An authored-empty slot has no retained semantic token and is
/// decisively `Invalid`. A slot headed by a `Function` token is
/// `OpaqueFunction` -- a structurally feasible position whose validity
/// depends on calculated-value semantics this leaf does not own -- only
...
/// generic whole-value-only Function name occupying this non-whole-value
/// position is decisively `Invalid` instead, mirroring the accepted
/// `scale`/`scaleX` boundary. A Function followed by further retained
/// material in the same slot is directly visible structural failure and
/// stays decisively `Invalid`.
///
/// A direct `Number` or `Percentage` qualifies regardless of
...
/// decisively `Invalid`, as is any slot carrying more than one retained
/// semantic token.
fn classify_transform_scaley_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformScaleYArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformScaleYArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => CssTransformScaleYArgumentClass::Invalid,
            Some(_) => CssTransformScaleYArgumentClass::OpaqueFunction,
            None => CssTransformScaleYArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Number { .. } => {
            CssTransformScaleYArgumentClass::Number(CssTransformScaleYArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Percentage { .. } => {
            CssTransformScaleYArgumentClass::Percentage(CssTransformScaleYArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        _ => CssTransformScaleYArgumentClass::Invalid,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformScaleZArgumentClass {
    Number(CssTransformScaleZArgumentEvidenceRef),
    Percentage(CssTransformScaleZArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `scaleZ()` argument slot against the
/// selected profile's single accepted shape -- a direct retained `<number>`
/// or a direct retained `<percentage>` (#663), mirroring
/// `classify_transform_scaley_argument` exactly except for the distinct
/// `CssTransformScaleZArgumentEvidenceRef` evidence type: `scaleZ()`
/// argument placement, `scaleY()` argument placement, `scaleX()` argument
/// placement, and `scale()` argument placement remain distinct semantic
/// roles even though they share the same direct scalar theorem under
/// current CSS Transforms Level 2 authority. `scaleZ()` shares its `Z`
/// suffix with `translateZ()`, but this classifier never reuses
/// `classify_transform_translatez_argument`: `translateZ()` accepts only a
/// direct exact-zero `Number` or a recognized-length `Dimension`, while
/// `scaleZ()` accepts any direct `Number` (no exact-zero restriction) or a
/// direct `Percentage`, so `scaleZ(10px)` is `Invalid` where
/// `translateZ(10px)` qualifies, and `scaleZ(2)` qualifies where
/// `translateZ(2)` is `Invalid`.
///
/// Whitespace and Comment trivia are excluded exactly as for `scaleY()`
/// arguments. An authored-empty slot has no retained semantic token and is
/// decisively `Invalid`. A slot headed by a `Function` token is
/// `OpaqueFunction` -- a structurally feasible position whose validity
/// depends on calculated-value semantics this leaf does not own -- only
...
/// generic whole-value-only Function name occupying this non-whole-value
/// position is decisively `Invalid` instead, mirroring the accepted
/// `scale`/`scaleX`/`scaleY` boundary. A Function followed by further
/// retained material in the same slot is directly visible structural
/// failure and stays decisively `Invalid`.
///
/// A direct `Number` or `Percentage` qualifies regardless of
/// `CssNumberType`; no range restriction and no machine-number conversion
/// is applied, so exact authored evidence stays authoritative for
...
/// decisively `Invalid`, as is any slot carrying more than one retained
/// semantic token.
fn classify_transform_scalez_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformScaleZArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformScaleZArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => CssTransformScaleZArgumentClass::Invalid,
            Some(_) => CssTransformScaleZArgumentClass::OpaqueFunction,
            None => CssTransformScaleZArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Number { .. } => {
            CssTransformScaleZArgumentClass::Number(CssTransformScaleZArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Percentage { .. } => {
            CssTransformScaleZArgumentClass::Percentage(CssTransformScaleZArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        _ => CssTransformScaleZArgumentClass::Invalid,
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformRotateArgumentClass {
    Angle(CssTransformRotateArgumentEvidenceRef),
    Zero(CssTransformRotateArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `rotate()` argument slot against the
/// selected profile's single accepted shape -- a direct retained `<angle>`
/// Dimension or a direct retained literal `<zero>` Number (#667), mirroring
/// `classify_transform_rotate3d_angle_argument` exactly except for the
/// distinct `CssTransformRotateArgumentEvidenceRef` evidence type:
/// `rotate()`'s single authored slot and `rotate3d()`'s positional
/// fourth-slot angle argument remain distinct semantic placements even
/// though both accept the same direct `<angle> | <zero>` scalar theorem
/// under current CSS Transforms Level 2 / CSS Values authority. `0` and
/// `0deg` remain distinct authored branches here exactly as for
/// `rotate3d()`: a `<zero>` is never synthesized into an `Angle`, a
/// recognized-angle `Dimension` is never collapsed into `Zero` merely
/// because its magnitude is zero, and this leaf never broadens the general
/// `<angle>` grammar to accept a unitless Number.
///
/// Whitespace and Comment trivia are excluded exactly as for `rotate3d()`'s
/// fourth slot. An authored-empty slot has no retained semantic token and
...
/// and no machine-number conversion is applied, so exact authored evidence
/// stays authoritative for membership.
fn classify_transform_rotate_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformRotateArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformRotateArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => CssTransformRotateArgumentClass::Invalid,
            Some(_) => CssTransformRotateArgumentClass::OpaqueFunction,
            None => CssTransformRotateArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Dimension { unit, .. } if is_css_angle_unit(unit) => {
            CssTransformRotateArgumentClass::Angle(CssTransformRotateArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Number { value, .. } if is_direct_zero_numeric_value(value) => {
            CssTransformRotateArgumentClass::Zero(CssTransformRotateArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        _ => CssTransformRotateArgumentClass::Invalid,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformRotateXArgumentClass {
    Angle(CssTransformRotateXArgumentEvidenceRef),
    Zero(CssTransformRotateXArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `rotateX()` argument slot against the
/// selected profile's single accepted shape -- a direct retained `<angle>`
/// Dimension or a direct retained literal `<zero>` Number (#669), mirroring
/// `classify_transform_rotate_argument` exactly except for the distinct
/// `CssTransformRotateXArgumentEvidenceRef` evidence type: `rotateX()`'s
/// single authored slot, `rotate()`'s single authored slot, and
/// `rotate3d()`'s positional fourth-slot angle argument remain distinct
/// semantic placements even though all three accept the same direct
...
/// here exactly as for `rotate()`/`rotate3d()`: a `<zero>` is never
/// synthesized into an `Angle`, a recognized-angle `Dimension` is never
/// collapsed into `Zero` merely because its magnitude is zero, and this
/// leaf never broadens the general `<angle>` grammar to accept a unitless
/// Number.
///
/// Whitespace and Comment trivia are excluded exactly as for `rotate()`'s
...
/// and no machine-number conversion is applied, so exact authored evidence
/// stays authoritative for membership.
fn classify_transform_rotatex_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformRotateXArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformRotateXArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => {
                CssTransformRotateXArgumentClass::Invalid
            }
            Some(_) => CssTransformRotateXArgumentClass::OpaqueFunction,
            None => CssTransformRotateXArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Dimension { unit, .. } if is_css_angle_unit(unit) => {
            CssTransformRotateXArgumentClass::Angle(CssTransformRotateXArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Number { value, .. } if is_direct_zero_numeric_value(value) => {
            CssTransformRotateXArgumentClass::Zero(CssTransformRotateXArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        _ => CssTransformRotateXArgumentClass::Invalid,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CssTransformRotateYArgumentClass {
    Angle(CssTransformRotateYArgumentEvidenceRef),
    Zero(CssTransformRotateYArgumentEvidenceRef),
    OpaqueFunction,
    Invalid,
}

/// Classifies one already-partitioned `rotateY()` argument slot against the
/// selected profile's single accepted shape -- a direct retained `<angle>`
/// Dimension or a direct retained literal `<zero>` Number (#671), mirroring
/// `classify_transform_rotatex_argument` exactly except for the distinct
/// `CssTransformRotateYArgumentEvidenceRef` evidence type: `rotateY()`'s
/// single authored slot, `rotateX()`'s single authored slot, `rotate()`'s
/// single authored slot, and `rotate3d()`'s positional fourth-slot angle
/// argument remain distinct semantic placements even though all four accept
/// the same direct `<angle> | <zero>` scalar theorem under current CSS
/// Transforms Level 2 / CSS Values authority. `0` and `0deg` remain distinct
/// authored branches here exactly as for `rotate()`/`rotateX()`/`rotate3d()`:
/// a `<zero>` is never synthesized into an `Angle`, a recognized-angle
...
///
/// Whitespace and Comment trivia are excluded exactly as for `rotateX()`'s
/// single slot. An authored-empty slot has no retained semantic token and
/// is decisively `Invalid`. A slot headed by a `Function` token is
/// `OpaqueFunction` -- a structurally feasible position whose validity
/// depends on calculated-value semantics this leaf does not own, so
...
/// and no machine-number conversion is applied, so exact authored evidence
/// stays authoritative for membership.
fn classify_transform_rotatey_argument(
    slot: &[CssLexicalItem],
    absolute_slot_start: usize,
) -> CssTransformRotateYArgumentClass {
    let mut tokens = slot
        .iter()
        .enumerate()
        .filter_map(|(relative_index, entry)| match entry {
    ...
    let Some((relative_index, first)) = tokens.next() else {
        return CssTransformRotateYArgumentClass::Invalid;
    };

    if matches!(first.kind(), CssTokenKind::Function(_)) {
        return match entire_function_name(slot) {
            Some(name) if is_whole_value_function(name) => {
                CssTransformRotateYArgumentClass::Invalid
            }
            Some(_) => CssTransformRotateYArgumentClass::OpaqueFunction,
            None => CssTransformRotateYArgumentClass::Invalid,
        };
    }
    ...
    match first.kind() {
        CssTokenKind::Dimension { unit, .. } if is_css_angle_unit(unit) => {
            CssTransformRotateYArgumentClass::Angle(CssTransformRotateYArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        CssTokenKind::Number { value, .. } if is_direct_zero_numeric_value(value) => {
            CssTransformRotateYArgumentClass::Zero(CssTransformRotateYArgumentEvidenceRef {
                lexical_item_index: absolute_slot_start + relative_index,
            })
        }
        _ => CssTransformRotateYArgumentClass::Invalid,
    }
}
...
enum CssTransformComponentClass {
    ...
    TranslateY(CssTransformTranslateYArgument),
    TranslateZ(CssTransformTranslateZArgumentEvidenceRef),
...
/// TranslateZ | ScaleX | ScaleY | ScaleZ | Scale3d | Rotate | RotateX`
...
///
/// A Function named `translate` ASCII-case-insensitively enters
/// `translate()` argument qualification the same way `scale()` does: a
/// directly visible slot count of zero or more than two is decisive
/// `Invalid` before any argument is classified, so `translate()`,
/// `translate(1px,2px,3px)`, and `translate(calc(1px),2px,3px)` all stay
...
///
/// A Function named `translatey` ASCII-case-insensitively enters
/// `translateY()` argument qualification, applying exactly the same
/// directly-visible-arity-first shell and single-slot classification as
/// `translateX()` (#655): a slot count other than one is decisive
/// `Invalid` before any argument is classified, so `translateY()` and
/// `translateY(10px,20px)` both stay invalid on directly visible arity.
/// Only at exactly one slot is the argument classified, accepting a direct
/// `Length` or `Percentage`, with the same decisive-argument-over-opaque-
/// sibling precedence as the other selected leaves. The qualified single
/// argument is carried directly by `CssTransformTranslateYArgument`, a
/// distinct type from `CssTransformTranslateXArgument` even though their
/// grammar is identical: `translateX()` and `translateY()` are kept as
/// separate semantic placements rather than merged into a generic
/// axis-function abstraction, so their evidence and representation never
/// leak into each other regardless of authored order.
///
/// A Function named `translatez` ASCII-case-insensitively enters
/// `translateZ()` argument qualification: a directly visible slot count
/// other than one is decisive `Invalid` before any argument is classified,
/// so `translateZ()` and `translateZ(10px,20px)` both stay invalid on
/// directly visible arity. Only at exactly one slot is the argument
/// classified, accepting a direct `Length` only -- unlike `translateX()`/
/// `translateY()`, a direct `Percentage` (including `0%`) is decisively
/// `Invalid` here rather than a qualified alternative, since `translateZ()`
/// admits only `<length>` (#657). The same decisive-argument-over-opaque-
/// sibling precedence applies as the other selected leaves. The qualified
/// single argument is carried directly by a bare
/// `CssTransformTranslateZArgumentEvidenceRef`, which has no
/// Length/Percentage `kind` to represent: a qualified `translateZ()`
/// argument can never resolve to a `Percentage` token, mirroring how the
/// `translate3d()` Z slot carries its evidence.
///
/// A Function named `scalex` ASCII-case-insensitively enters `scaleX()`
/// argument qualification: a directly visible slot count other than one is
/// decisive `Invalid` before any argument is classified, so `scaleX()` and
/// `scaleX(1,2)` both stay invalid on directly visible arity. Only at
/// exactly one slot is the argument classified, accepting a direct
/// `Number` or `Percentage` under current CSS Transforms Level 2 authority
/// -- reusing the accepted `scale()` (#645) Number/Percentage theorem --
/// with the same decisive-argument-over-opaque-sibling precedence as the
/// other selected leaves (#659). The qualified single argument is carried
/// directly by `CssTransformScaleXArgument`, a distinct type from
/// `CssTransformScaleArgument` even though their grammar is identical:
...
/// argument qualification: a directly visible slot count other than one is
/// decisive `Invalid` before any argument is classified, so `scaleY()` and
/// `scaleY(1,2)` both stay invalid on directly visible arity. Only at
/// exactly one slot is the argument classified, accepting a direct `Number`
/// or `Percentage` under current CSS Transforms Level 2 authority -- reusing
/// the accepted `scale()` (#645) / `scaleX()` (#659) Number/Percentage
/// theorem -- with the same decisive-argument-over-opaque-sibling
/// precedence as the other selected leaves (#661). The qualified single
/// argument is carried directly by `CssTransformScaleYArgument`, a distinct
/// type from `CssTransformScaleArgument` and `CssTransformScaleXArgument`
/// even though their grammar is identical: `scaleY()`, `scaleX()`, and
...
/// argument qualification: a directly visible slot count other than one is
/// decisive `Invalid` before any argument is classified, so `scaleZ()` and
/// `scaleZ(1,2)` both stay invalid on directly visible arity. Only at
/// exactly one slot is the argument classified, accepting a direct `Number`
/// or `Percentage` under current CSS Transforms Level 2 authority -- reusing
/// the accepted `scale()` (#645) / `scaleX()` (#659) / `scaleY()` (#661)
/// Number/Percentage theorem -- with the same decisive-argument-over-opaque-
/// sibling precedence as the other selected leaves (#663). The qualified
/// single argument is carried directly by `CssTransformScaleZArgument`, a
/// distinct type from `CssTransformScaleArgument`, `CssTransformScaleXArgument`,
/// and `CssTransformScaleYArgument` even though their grammar is identical:
/// `scaleZ()`, `scaleY()`, `scaleX()`, and `scale()` are kept as separate
/// semantic placements rather than merged into a generic scale-axis
/// abstraction, so their evidence and representation never leak into each
/// other regardless of authored order. `scaleZ()` also shares its `Z` suffix
/// with `translateZ()`, but the two are never conflated: `translateZ()`
/// admits only a direct exact-zero `Number` or a recognized-length
/// `Dimension`, while `scaleZ()` admits any direct `Number` or a direct
...
/// each of them.
///
/// A Function named `rotate` ASCII-case-insensitively enters `rotate()`
/// argument qualification: a directly visible slot count other than one is
/// decisive `Invalid` before any argument is classified, so `rotate()` and
/// `rotate(0,90deg)` both stay invalid on directly visible arity. Only at
/// exactly one slot is the argument classified, accepting a direct `<angle>`
/// Dimension or a direct literal `<zero>` Number under current CSS
/// Transforms Level 2 / CSS Values authority -- reusing the exact
/// `Angle`/`Zero` scalar theorem already accepted for `rotate3d()`'s fourth
/// slot (#649), so `calc(0)` is never direct `<zero>` here either (#667).
/// The qualified argument is carried by `CssTransformRotateFunction`, a
/// distinct type from `CssTransformRotate3dFunction`: `rotate()` never
/// shares representation with `rotate3d()`, with the accepted longhand
/// `rotate` property qualifier (#604), or with a future `rotateX()`/
...
fn classify_transform_component(
    ...
    if name.eq_ignore_ascii_case("translate") {
        ...
        for slot in slots {
            let absolute_slot_start = absolute_component_start + slot.start;
            match classify_transform_translate_argument(&component[slot], absolute_slot_start) {
                CssTransformTranslateArgumentClass::Length(evidence_ref) => {
                    arguments.push(CssTransformTranslateArgument {
                        kind: CssTransformTranslateArgumentKind::Length,
                        evidence_ref,
                    });
                }
                CssTransformTranslateArgumentClass::Percentage(evidence_ref) => {
                    arguments.push(CssTransformTranslateArgument {
                        kind: CssTransformTranslateArgumentKind::Percentage,
                        evidence_ref,
                    });
                }
                CssTransformTranslateArgumentClass::OpaqueFunction => has_opaque_argument = true,
                CssTransformTranslateArgumentClass::Invalid => {
                    return CssTransformComponentClass::Invalid;
                }
            }
        ...
        let arguments = match arguments.len() {
            1 => CssTransformTranslateArguments::One(arguments[0]),
            2 => CssTransformTranslateArguments::Two(arguments[0], arguments[1]),
            _ => unreachable!("translate slot count is already bounded to 1..=2 above"),
        };
    ...
    if name.eq_ignore_ascii_case("translatey") {
        ...
        let slot_start = absolute_component_start + slots[0].start;
        let argument_class =
            classify_transform_translatey_argument(&component[slots[0].clone()], slot_start);

        let argument = match argument_class {
            CssTransformTranslateYArgumentClass::Length(evidence_ref) => {
                CssTransformTranslateYArgument {
                    kind: CssTransformTranslateYArgumentKind::Length,
                    evidence_ref,
                }
            }
            CssTransformTranslateYArgumentClass::Percentage(evidence_ref) => {
                CssTransformTranslateYArgument {
                    kind: CssTransformTranslateYArgumentKind::Percentage,
                    evidence_ref,
                }
            }
            CssTransformTranslateYArgumentClass::OpaqueFunction => {
                return CssTransformComponentClass::OpaqueTransformArgument;
            }
            CssTransformTranslateYArgumentClass::Invalid => {
                return CssTransformComponentClass::Invalid;
        ...
        return CssTransformComponentClass::TranslateY(argument);
    }

    if name.eq_ignore_ascii_case("translatez") {
        let slots = transform_function_body_slot_ranges(component);
        if slots.len() != 1 {
            return CssTransformComponentClass::Invalid;
        }
        ...
        let slot_start = absolute_component_start + slots[0].start;
        let argument_class =
            classify_transform_translatez_argument(&component[slots[0].clone()], slot_start);

        let argument = match argument_class {
            CssTransformTranslateZArgumentClass::Length(evidence_ref) => evidence_ref,
            CssTransformTranslateZArgumentClass::OpaqueFunction => {
                return CssTransformComponentClass::OpaqueTransformArgument;
            }
            CssTransformTranslateZArgumentClass::Invalid => {
                return CssTransformComponentClass::Invalid;
        ...
        return CssTransformComponentClass::TranslateZ(argument);
    }

    if name.eq_ignore_ascii_case("scalex") {
        let slots = transform_function_body_slot_ranges(component);
        if slots.len() != 1 {
            return CssTransformComponentClass::Invalid;
        }
        ...
        let slot_start = absolute_component_start + slots[0].start;
        let argument_class =
            classify_transform_scalex_argument(&component[slots[0].clone()], slot_start);

        let argument = match argument_class {
            CssTransformScaleXArgumentClass::Number(evidence_ref) => CssTransformScaleXArgument {
                kind: CssTransformScaleXArgumentKind::Number,
                evidence_ref,
            },
            CssTransformScaleXArgumentClass::Percentage(evidence_ref) => {
                CssTransformScaleXArgument {
                    kind: CssTransformScaleXArgumentKind::Percentage,
                    evidence_ref,
                }
            }
            CssTransformScaleXArgumentClass::OpaqueFunction => {
                return CssTransformComponentClass::OpaqueTransformArgument;
            }
            CssTransformScaleXArgumentClass::Invalid => {
                return CssTransformComponentClass::Invalid;
            }
        };

        return CssTransformComponentClass::ScaleX(argument);
    }

    if name.eq_ignore_ascii_case("scaley") {
        let slots = transform_function_body_slot_ranges(component);
        if slots.len() != 1 {
            return CssTransformComponentClass::Invalid;
        }
        ...
        let slot_start = absolute_component_start + slots[0].start;
        let argument_class =
            classify_transform_scaley_argument(&component[slots[0].clone()], slot_start);

        let argument = match argument_class {
            CssTransformScaleYArgumentClass::Number(evidence_ref) => CssTransformScaleYArgument {
                kind: CssTransformScaleYArgumentKind::Number,
                evidence_ref,
            },
            CssTransformScaleYArgumentClass::Percentage(evidence_ref) => {
                CssTransformScaleYArgument {
                    kind: CssTransformScaleYArgumentKind::Percentage,
                    evidence_ref,
                }
            }
            CssTransformScaleYArgumentClass::OpaqueFunction => {
                return CssTransformComponentClass::OpaqueTransformArgument;
            }
            CssTransformScaleYArgumentClass::Invalid => {
                return CssTransformComponentClass::Invalid;
            }
        };

        return CssTransformComponentClass::ScaleY(argument);
    }

    if name.eq_ignore_ascii_case("scalez") {
        let slots = transform_function_body_slot_ranges(component);
        if slots.len() != 1 {
            return CssTransformComponentClass::Invalid;
        }
        ...
        let slot_start = absolute_component_start + slots[0].start;
        let argument_class =
            classify_transform_scalez_argument(&component[slots[0].clone()], slot_start);

        let argument = match argument_class {
            CssTransformScaleZArgumentClass::Number(evidence_ref) => CssTransformScaleZArgument {
                kind: CssTransformScaleZArgumentKind::Number,
                evidence_ref,
            },
            CssTransformScaleZArgumentClass::Percentage(evidence_ref) => {
                CssTransformScaleZArgument {
                    kind: CssTransformScaleZArgumentKind::Percentage,
                    evidence_ref,
                }
            }
            CssTransformScaleZArgumentClass::OpaqueFunction => {
                return CssTransformComponentClass::OpaqueTransformArgument;
            }
            CssTransformScaleZArgumentClass::Invalid => {
                return CssTransformComponentClass::Invalid;
            }
        };
    ...
    if name.eq_ignore_ascii_case("rotate") {
        ...
        let slot_start = absolute_component_start + slots[0].start;
        let argument_class =
            classify_transform_rotate_argument(&component[slots[0].clone()], slot_start);

        let argument = match argument_class {
            CssTransformRotateArgumentClass::Angle(evidence_ref) => CssTransformRotateArgument {
                kind: CssTransformRotateArgumentKind::Angle,
                evidence_ref,
            },
            CssTransformRotateArgumentClass::Zero(evidence_ref) => CssTransformRotateArgument {
                kind: CssTransformRotateArgumentKind::Zero,
                evidence_ref,
            },
            CssTransformRotateArgumentClass::OpaqueFunction => {
                return CssTransformComponentClass::OpaqueTransformArgument;
            }
            CssTransformRotateArgumentClass::Invalid => {
                return CssTransformComponentClass::Invalid;
            }
        };

        return CssTransformComponentClass::Rotate(argument);
    }

    if name.eq_ignore_ascii_case("rotatex") {
        let slots = transform_function_body_slot_ranges(component);
        if slots.len() != 1 {
            return CssTransformComponentClass::Invalid;
        }
        ...
        let slot_start = absolute_component_start + slots[0].start;
        let argument_class =
            classify_transform_rotatex_argument(&component[slots[0].clone()], slot_start);

        let argument = match argument_class {
            CssTransformRotateXArgumentClass::Angle(evidence_ref) => CssTransformRotateXArgument {
                kind: CssTransformRotateXArgumentKind::Angle,
                evidence_ref,
            },
            CssTransformRotateXArgumentClass::Zero(evidence_ref) => CssTransformRotateXArgument {
                kind: CssTransformRotateXArgumentKind::Zero,
                evidence_ref,
            },
            CssTransformRotateXArgumentClass::OpaqueFunction => {
                return CssTransformComponentClass::OpaqueTransformArgument;
            }
            CssTransformRotateXArgumentClass::Invalid => {
                return CssTransformComponentClass::Invalid;
        ...
        return CssTransformComponentClass::RotateX(argument);
    }

    if name.eq_ignore_ascii_case("rotatey") {
        let slots = transform_function_body_slot_ranges(component);
        if slots.len() != 1 {
            return CssTransformComponentClass::Invalid;
        }
        ...
        let slot_start = absolute_component_start + slots[0].start;
        let argument_class =
            classify_transform_rotatey_argument(&component[slots[0].clone()], slot_start);

        let argument = match argument_class {
            CssTransformRotateYArgumentClass::Angle(evidence_ref) => CssTransformRotateYArgument {
                kind: CssTransformRotateYArgumentKind::Angle,
                evidence_ref,
            },
            CssTransformRotateYArgumentClass::Zero(evidence_ref) => CssTransformRotateYArgument {
                kind: CssTransformRotateYArgumentKind::Zero,
                evidence_ref,
            },
            CssTransformRotateYArgumentClass::OpaqueFunction => {
                return CssTransformComponentClass::OpaqueTransformArgument;
            }
            CssTransformRotateYArgumentClass::Invalid => {
                return CssTransformComponentClass::Invalid;
...
/// `matrix()`/`scale()`/`translate3d()`/`rotate3d()`/`translate()`/
/// `translateX()`/`translateY()`/`translateZ()`/`scaleX()` components in
/// exact authored order via `CssTransformFunction`.
///
...
fn qualify_transform_value(
    ...
    for class in component_classes {
        match class {
            ...
            CssTransformComponentClass::TranslateY(argument) => {
                functions.push(CssTransformFunction::TranslateY(
                    CssTransformTranslateYFunction { argument },
                ));
            }
            CssTransformComponentClass::TranslateZ(argument) => {
                functions.push(CssTransformFunction::TranslateZ(
                    CssTransformTranslateZFunction { argument },
                ));
            }
            CssTransformComponentClass::ScaleX(argument) => {
                functions.push(CssTransformFunction::ScaleX(CssTransformScaleXFunction {
                    argument,
                }));
            }
            CssTransformComponentClass::ScaleY(argument) => {
                functions.push(CssTransformFunction::ScaleY(CssTransformScaleYFunction {
                    argument,
                }));
            }
            CssTransformComponentClass::ScaleZ(argument) => {
            ...
            CssTransformComponentClass::Rotate(argument) => {
                functions.push(CssTransformFunction::Rotate(CssTransformRotateFunction {
                    argument,
                }));
            }
            CssTransformComponentClass::RotateX(argument) => {
                functions.push(CssTransformFunction::RotateX(CssTransformRotateXFunction {
                    argument,
                }));
            }
