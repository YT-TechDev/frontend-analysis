...
// 0..3 facts per binding/declarator, consumed in exact authored reference
// order without reordering or deduplication. This consumer's own logic is
// unchanged; #795/PR #796 independently proves the underlying bounded
// three-fact theorem. These tests seal only that this consumer composes it
// correctly across top-level `var`, Block `var`, declarator lists, and
// duplicate occurrences, with no new relation type or precedence.

#[test]
fn three_identifier_reference_additive_initializer_composes_for_top_level_var() {
    let (_, script) = recognized_variable("let a; var b; let c; var x = a+b+c;");
    let analysis = accepted_analysis(&script);
    let relations = analysis.relations();
    assert_eq!(
        relations
...
#[test]
fn three_identifier_reference_additive_initializer_composes_for_one_level_block_var() {
    ...
    let (_, script) = recognized_variable("let c; var b; { let a; var x = a+b+c; }");
    let analysis = accepted_analysis(&script);
    let relations = analysis.relations();
    assert_eq!(relations.len(), 3);
    assert_eq!(relations[0].semantic_name(), "a");
...
#[test]
fn three_identifier_reference_additive_initializer_does_not_deduplicate_equal_semantic_names_for_var()
 {
    let (_, script) = recognized_variable("let a; var x = a + a + a;");
    let analysis = accepted_analysis(&script);
    let relations = analysis.relations();
    assert_eq!(relations.len(), 3);
    for relation in relations {
...
#[test]
fn three_identifier_reference_additive_initializer_composes_with_outer_declarator_list_cardinality_for_var()
 {
    // Issue #797 outer `1..N` x inner `1..3` composition: `x=a, y=b+c,
    // z=d+e+f` composes as `x.one, y.first, y.second, z.first, z.second,
    // z.third`, in exact authored declarator and operand order.
    let (_, script) =
        recognized_variable("let a; let b; let c; let d; let e; let f; var x=a, y=b+c, z=d+e+f;");
    let analysis = accepted_analysis(&script);
    let relations = analysis.relations();
    assert_eq!(
        relations
...
// consumer's own logic. These tests seal only that this consumer composes
// it correctly across top-level `var`, one-level Block `var`, and duplicate
// occurrences, in exact authored order, with no new relation type.

#[test]
fn optional_unary_many_identifier_reference_additive_initializer_composes_for_top_level_var() {
    let (_, script) = recognized_variable("let a; var b; let c; let d; var x = +a+-b+c- -d;");
    let analysis = accepted_analysis(&script);
    let relations = analysis.relations();
    assert_eq!(
...
#[test]
fn optional_unary_many_identifier_reference_additive_initializer_composes_for_one_level_block_var()
{
    let (_, script) = recognized_variable("let c; let d; var b; { let a; var x = +a+-b+c- -d; }");
    let analysis = accepted_analysis(&script);
    let relations = analysis.relations();
...
#[test]
fn optional_unary_many_identifier_reference_additive_initializer_does_not_deduplicate_equal_semantic_names_for_var()
 {
    let (_, script) = recognized_variable("let a; var x = +a+-a+a- -a;");
    let analysis = accepted_analysis(&script);
    let relations = analysis.relations();
...
// --- Issue #799 (per #688 comment 5775218176): exactly-three
// IdentifierReference additive free-standing `ExpressionStatement` use-site
// correspondence. Each retained fact of a
// `SelectedFreeStandingIdentifierReferenceUseSite::Three` occurrence is an
// independent correspondence query, emitting one relation per fact in exact
// authored item order, then exact authored operand order within each item
// -- the existing `for fact in use_site.body().facts()` consumer composes
// this naturally, with no new relation type, meaning, sorting, or
// deduplication. ---

#[test]
fn three_operand_use_site_per_operand_correspondence_top_level() {
    // `let c; let a; var b; a+b+c;`: `a`/`c` resolve to lexical bindings,
    // `b` to the Script-wide var contributor -- three independent
    // per-operand queries, never one shared result for the whole additive
    // use-site.
    ...
    let (binding, region) = relations[0]
        .correspondence()
        .selected_lexical_binding()
        .expect("first operand must resolve to the lexical binding a");
    assert_eq!(binding.fragment(), "a");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::TopLevel
    ...
    let contributors = relations[1]
        .correspondence()
        .var_contributors()
        .expect("second operand must resolve to the Script-wide var contributor b");
    assert_eq!(contributors.len(), 1);

    assert_eq!(relations[2].semantic_name(), "c");
    let (binding, region) = relations[2]
        .correspondence()
        .selected_lexical_binding()
        .expect("third operand must resolve to the lexical binding c");
    assert_eq!(binding.fragment(), "c");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::TopLevel
    ));
...
#[test]
fn three_operand_use_site_per_operand_correspondence_block() {
    // `let c; var b; { let a; a+b+c; }`: `a` resolves to the current-Block
    // lexical binding, `b` to the existing var-contributor path, `c` to the
    // TopLevel lexical binding -- each independently uses existing
    ...
    let (binding, region) = relations[0]
        .correspondence()
        .selected_lexical_binding()
        .expect("first operand must resolve to the current-Block lexical binding a");
    assert_eq!(binding.fragment(), "a");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::Block(_)
    ...
    let (binding, region) = relations[2]
        .correspondence()
        .selected_lexical_binding()
        .expect("third operand must fall back to the TopLevel lexical binding c");
    assert_eq!(binding.fragment(), "c");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::TopLevel
...
#[test]
fn three_operand_use_site_duplicate_operands_top_level() {
    let (_, script) = recognized_reference_use("let a;\na+a+a;");
    let analysis = accepted_use_site_analysis(&script);
    let relations = analysis.relations();
    assert_eq!(
    ...
    assert_eq!(relations[0].semantic_name(), "a");
    assert_eq!(relations[1].semantic_name(), "a");
    assert_eq!(relations[2].semantic_name(), "a");
    assert!(relations[0].reference().range().start() < relations[1].reference().range().start());
    assert!(relations[1].reference().range().start() < relations[2].reference().range().start());
}

#[test]
fn three_operand_use_site_item_order_and_operand_order_composition_top_level() {
    // `a; b+c; d+e+f; g;`: authored item order, then authored operand order
    // within each item -- never later source sorting.
    let (_, script) = recognized_reference_use("a;\nb+c;\nd+e+f;\ng;");
    let analysis = accepted_use_site_analysis(&script);
    let names: Vec<&str> = analysis
        .relations()
...
#[test]
fn three_operand_use_site_item_order_and_operand_order_composition_block() {
    let (_, script) = recognized_block_reference_use("{\n    a;\n    b+c;\n    d+e+f;\n    g;\n}");
    let analysis = accepted_block_use_site_analysis(&script);
    let names: Vec<&str> = analysis
        .relations()
...
#[test]
fn three_operand_use_site_initializer_and_free_standing_surfaces_remain_separate() {
    let (_, script) = recognized_reference_use("let x=a+b+c;\nd+e+f;");
    ...
    let initializer_names: Vec<&str> = initializer_analysis
        ...
        .map(|relation| relation.semantic_name())
        .collect();
    assert_eq!(initializer_names, ["a", "b", "c"]);
    for relation in initializer_analysis.relations() {
        assert_eq!(relation.containing_binding().fragment(), "x");
    }
    ...
    let use_site_names: Vec<&str> = use_site_analysis
        ...
        .map(|relation| relation.semantic_name())
        .collect();
    assert_eq!(use_site_names, ["d", "e", "f"]);
}

#[test]
fn three_operand_use_site_known_static_rejection_suppresses_relation_construction() {
    let source = source("let a;\n{ var a; b+c+d; }");
    let script = match recognize_selected_lexical_slice(&source) {
        SelectedLexicalSliceOutcome::RecognizedBlockReferenceUseEnabledSlice(script) => script,
        other => panic!("expected Block-reference-use-enabled recognition, got {other:?}"),
    ...
    // No relation surface is reachable without an accepted witness: none of
    // the three operands of the locally valid `b+c+d;` use-site ever
    // publish a relation once the existing lexical/var static rejection
    // wins.
}

// --- Issue #807 (per #688 comment 5780964757): unbounded ordered 2..N
// IdentifierReference additive free-standing `ExpressionStatement` use-site
// correspondence. Each retained fact of a
// `SelectedFreeStandingIdentifierReferenceUseSite::Many` occurrence is an
// independent correspondence query, emitting one relation per fact in exact
// authored item order, then exact authored operand order within each item
// -- the existing `for fact in use_site.body().facts()` consumer composes
// this naturally, with no new relation type, meaning, sorting, or
// deduplication. Production semantic logic in this module is unchanged by
// this Issue; only this test module widens. ---

#[test]
fn many_operand_use_site_per_operand_correspondence_top_level() {
    // `let d; let a; var c; let b; a+b+c+d;`: declaration order (d, a, c,
    // b) differs from authored reference order (a, b, c, d) -- four
    // independent per-operand queries, never one shared result for the
    // whole additive use-site.
    let (_, script) = recognized_reference_use("let d;\nlet a;\nvar c;\nlet b;\na+b+c+d;");
    let analysis = accepted_use_site_analysis(&script);
    let relations = analysis.relations();
    assert_eq!(relations.len(), 4);
    ...
    let (binding, region) = relations[1]
        .correspondence()
        .selected_lexical_binding()
        .expect("second operand must resolve to the lexical binding b");
    assert_eq!(binding.fragment(), "b");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::TopLevel
    ...
    assert_eq!(relations[2].semantic_name(), "c");
    let contributors = relations[2]
        .correspondence()
        .var_contributors()
        .expect("third operand must resolve to the Script-wide var contributor c");
    assert_eq!(contributors.len(), 1);

    assert_eq!(relations[3].semantic_name(), "d");
    let (binding, region) = relations[3]
        .correspondence()
        .selected_lexical_binding()
        .expect("fourth operand must resolve to the lexical binding d");
    assert_eq!(binding.fragment(), "d");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::TopLevel
    ));
...
#[test]
fn many_operand_use_site_per_operand_correspondence_block() {
    // `let d; var c; let e; { let a; let b; a+b+c+d+e; }`: five operands
    // mixing current-Block lexical bindings (a, b), the Script-wide var
    // contributor (c), and TopLevel lexical fallback (d, e).
    let (_, script) =
        recognized_block_reference_use("let d;\nvar c;\nlet e;\n{ let a;\nlet b;\na+b+c+d+e; }");
    let analysis = accepted_block_use_site_analysis(&script);
    let relations = analysis.relations();
    ...
    let (binding, region) = relations[1]
        .correspondence()
        .selected_lexical_binding()
        .expect("second operand must resolve to the current-Block lexical binding b");
    assert_eq!(binding.fragment(), "b");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::Block(_)
    ...
    let (binding, region) = relations[3]
        .correspondence()
        .selected_lexical_binding()
        .expect("fourth operand must fall back to the TopLevel lexical binding d");
    assert_eq!(binding.fragment(), "d");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::TopLevel
    ...
    assert_eq!(relations[4].semantic_name(), "e");
    let (binding, region) = relations[4]
        .correspondence()
        .selected_lexical_binding()
        .expect("fifth operand must fall back to the TopLevel lexical binding e");
    assert_eq!(binding.fragment(), "e");
    assert!(matches!(
        region,
        SelectedVariableStatementNameCorrespondenceRegion::TopLevel
    ));
...
#[test]
fn many_operand_use_site_duplicate_operands_top_level() {
    let (_, script) = recognized_reference_use("let a;\na+a+a+a+a;");
    let analysis = accepted_use_site_analysis(&script);
    let relations = analysis.relations();
    assert_eq!(
...
#[test]
fn one_two_three_many_one_operand_use_site_item_order_and_operand_order_composition_top_level() {
    // `a; b+c; d+e+f; g+h+i+j; k;`: authored item order, then authored
    // operand order within each item -- never later source sorting, and
    // `Many` composes exactly like every earlier bounded cardinality.
    let (_, script) = recognized_reference_use("a;\nb+c;\nd+e+f;\ng+h+i+j;\nk;");
    let analysis = accepted_use_site_analysis(&script);
    let names: Vec<&str> = analysis
        .relations()
...
#[test]
fn many_operand_use_site_known_static_rejection_suppresses_relation_construction() {
    let source = source("let a;\n{ var a; b+c+d+e; }");
    let script = match recognize_selected_lexical_slice(&source) {
        SelectedLexicalSliceOutcome::RecognizedBlockReferenceUseEnabledSlice(script) => script,
        other => panic!("expected Block-reference-use-enabled recognition, got {other:?}"),
    ...
    // No relation surface is reachable without an accepted witness: none of
    // the four operands of the locally valid `b+c+d+e;` use-site ever
    // publish a relation once the existing lexical/var static rejection
    // wins.
}
