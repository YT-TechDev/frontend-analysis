...
//! (plain fractional `DecimalLiteral` initializer), composing the accepted
//! post-#697 required-rule theorem (Block `var` decimal-initializer
//! composition) with the new Issue #727 atom.
//!
...
//! directly, and executably proves that adding the plain fractional
...
const HISTORICAL_POST_697_COMPLETION: &str =
...
/// This successor reaches no new Early Error identity: the plain fractional
...
/// identical in membership, because Issue #727 widens only the initializer
...
// became reachable, this is exactly the frozen post-#697 complement,
...
fn fixed_envelope_and_checkpoint_lineage_are_exact() {
    ...
    assert!(HISTORICAL_POST_697_COMPLETION.contains("POST_697_REQUIRED_RULE_IDS"));
    assert!(
        HISTORICAL_POST_697_COMPLETION
            .contains("assert_eq!(POST_697_REQUIRED_RULE_IDS.len(), 10);")
    );
    assert!(
        HISTORICAL_POST_697_COMPLETION
            .contains("assert_eq!(POST_697_COMPLEMENT_RULE_IDS.len(), 183);")
    );
...
#[test]
fn post_697_historical_checkpoint_is_preserved_not_rewritten() {
    ...
    assert!(HISTORICAL_POST_697_COMPLETION.contains("NEWLY_REACHABLE_RULE_IDS: &[&str] = &[];"));
    assert!(HISTORICAL_POST_697_COMPLETION.contains("assert_eq!(required_active, 10);"));
    assert!(HISTORICAL_POST_697_COMPLETION.contains("assert_eq!(complement_active, 173);"));
    assert!(HISTORICAL_POST_697_COMPLETION.contains("assert_eq!(complement_inactive, 10);"));
}
...
fn no_new_early_error_identity_becomes_reachable() {
    ...
    assert_eq!(
        previous, current,
        "the required-rule set is unchanged by Issue #727"
    );
...
/// Falsifies the wrong model that motivated this remediation: `required`
/// is not `RuleUnitKind::EnvelopeInactiveRule`, and `complement` is not
/// `RuleUnitKind::NormativeRule`. `RuleUnitKind` classifies applicability
/// under the frozen envelope; `required`/`complement` classify reachability
/// under the currently accepted selected slice. They are independent
/// partitions that happen to share the totals 10 and 183.
#[test]
