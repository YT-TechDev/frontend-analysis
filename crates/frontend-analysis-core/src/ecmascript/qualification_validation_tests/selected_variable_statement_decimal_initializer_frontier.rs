...
const POSITIVE_FIXTURES: &[PositiveFixture] = &[
    ...
    PositiveFixture {
        ...
        declarators: &[
            DeclaratorExpectation {
                binding: Range::new(4, 6),
                semantic_name: "é",
                decimal_rhs: Some(Range::new(7, 8)),
            },
            DeclaratorExpectation {
                binding: Range::new(9, 16),
                semantic_name: "e\u{301}",
                decimal_rhs: Some(Range::new(17, 18)),
            },
        ],
        terminator: Terminator::AuthoredSemicolon,
    },
    PositiveFixture {
        id: "non-strict-let-binding",
        source: "var a=1, let=2;",
        declarators: &[
            DeclaratorExpectation {
                binding: Range::new(4, 5),
                semantic_name: "a",
            ...
            DeclaratorExpectation {
                binding: Range::new(9, 12),
                semantic_name: "let",
                decimal_rhs: Some(Range::new(13, 14)),
            },
        ],
        terminator: Terminator::AuthoredSemicolon,
    },
...
const COLLISION_FIXTURES: &[CollisionFixture] = &[
    CollisionFixture {
        ...
        var: Range::new(15, 16),
        completing: Range::new(15, 16),
    },
    CollisionFixture {
        source: "let a,b; var b=1,a=2;",
        lexical: Range::new(6, 7),
        var: Range::new(13, 14),
        completing: Range::new(13, 14),
    },
    CollisionFixture {
        source: "var b=1,a=2; let a,b;",
        lexical: Range::new(17, 18),
        var: Range::new(8, 9),
        completing: Range::new(17, 18),
    },
    CollisionFixture {
...
const CONTRIBUTOR_FIXTURES: &[ContributorFixture] = &[
    ...
    ContributorFixture {
        source: "var a=1,b; var a=2; let x=a;",
        contributors: &[Range::new(4, 5), Range::new(15, 16)],
    },
];
...
fn positives_pin_per_declarator_presence_exact_rhs_ranges_and_one_to_n_order() {
    for (fixture_index, fixture) in POSITIVE_FIXTURES.iter().enumerate() {
        ...
        assert!(
            fixture
                .declarators
                .iter()
                .any(|declarator| declarator.decimal_rhs.is_some())
        );
    ...
    for (id, expected_count) in [
        ("single-zero-authored", 1),
        ("two-initialized", 2),
        ("three-mixed", 3),
    ] {
        let fixture = POSITIVE_FIXTURES
            .iter()
...
fn mixed_initialized_and_uninitialized_declarators_remain_distinct() {
    for (id, expected) in [
        ...
        let fixture = POSITIVE_FIXTURES
            ...
            .find(|fixture| fixture.id == id)
            .unwrap();
        assert_eq!(fixture.declarators.len(), expected.len());
        for (declarator, expected_present) in fixture.declarators.iter().zip(expected.iter()) {
            assert_eq!(declarator.decimal_rhs.is_some(), *expected_present);
        }
    }
}

#[test]
fn semantic_identity_preserves_direct_escape_equality_without_unicode_normalization() {
    ...
    let distinct = POSITIVE_FIXTURES
        .iter()
        .find(|fixture| fixture.id == "canonical-distinct")
        .unwrap();
    assert_ne!(
        distinct.declarators[0].semantic_name,
        distinct.declarators[1].semantic_name
    );
}
...
fn binding_grammar_and_static_routes_remain_distinct() {
    let escaped = BINDING_BOUNDARIES
        .iter()
        .find(|fixture| fixture.source == r"var \u0069f=1;")
        .unwrap();
    assert_eq!(
        escaped.disposition,
    ...
    let malformed = BINDING_BOUNDARIES
        .iter()
        .find(|fixture| fixture.source == r"var a,\u{}=1;")
        .unwrap();
    assert_eq!(
    ...
    let equal = BINDING_BOUNDARIES
        .iter()
        .find(|fixture| fixture.source == r"var a,\u0061=1;")
        .unwrap();
    assert_eq!(equal.disposition, Disposition::SelectedAcceptedIncomplete);
    assert!(
        BINDING_BOUNDARIES
            .iter()
            .any(|fixture| fixture.source == "var if=1;"
...
fn tier_priority_remains_above_authored_order_across_initializer_bearing_var() {
    ...
    assert_anchor(
        PRECEDENCE_FIXTURE.source,
        PRECEDENCE_FIXTURE.primary.subject,
        ISSUE_ID,
    );
    assert_anchor(
        PRECEDENCE_FIXTURE.source,
...
#[test]
fn ee36_collision_selection_ignores_initializer_content_and_preserves_authored_order() {
    ...
    assert_eq!(
        slice(COLLISION_FIXTURES[0].source, COLLISION_FIXTURES[0].var),
        "b"
    );
...
#[test]
fn correspondence_remains_authored_contributor_provenance_not_initializer_semantics() {
    ...
    let repeated = CONTRIBUTOR_FIXTURES
        .iter()
        .find(|fixture| fixture.source == "var a=1,a; let x=a;")
        .unwrap();
    assert_eq!(repeated.contributors, &[Range::new(4, 5), Range::new(8, 9)]);
}
...
fn terminator_semantics_remain_statement_owned() {
    assert!(
        POSITIVE_FIXTURES
            .iter()
            .any(|fixture| fixture.source == "var a=1\n, b=2;"
    ...
    assert!(
        OUT_OF_SCOPE_BOUNDARIES
            .iter()
            .any(|fixture| fixture.source == "var a=1, /*comment*/ b;"
                && fixture.disposition == Disposition::UnsupportedCoverage)
    );
}
...
fn rule_reachability_lifecycle_and_positive_partition_remain_unchanged() {
    ...
    let active_count = RULE_UNITS
        .iter()
        .filter(|rule| rule.kind == RuleUnitKind::NormativeRule)
        .count();
    let inactive_count = RULE_UNITS
        .iter()
        .filter(|rule| rule.kind == RuleUnitKind::EnvelopeInactiveRule)
        .count();
    let sentinel_count = RULE_UNITS
        .iter()
...
fn validation_remains_candidate_independent_runtime_negative_and_representation_neutral() {
    for forbidden in [
        ...
        assert!(
            ...
            "forbidden validation dependency or commitment: {forbidden}"
        );
    }
    assert!(
