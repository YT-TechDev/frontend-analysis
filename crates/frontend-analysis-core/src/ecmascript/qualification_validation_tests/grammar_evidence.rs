...
//!
//! This module extends validation evidence only. It deliberately does not call
//! the selected production lexical/static-semantics/aggregate candidate.
...
fn gold(gold_id: &str) -> GoldFixture {
    fixtures()
        ...
        .unwrap_or_else(|| panic!("missing grammar-evidence gold fixture {gold_id}"))
}

fn authored_fragment(source: &str, range: GoldRange) -> &str {
...
fn assert_core_source_anchor(source: &str, range: GoldRange) {
...
fn existing_syntax_gold_remains_subjectless_while_grammar_evidence_is_source_backed() {
    ...
    for fixture in BASE_GRAMMAR_EVIDENCE {
        ...
        let aggregate = gold(gold_id);
        assert_eq!(aggregate.source, fixture.source);
        assert_eq!(aggregate.applicability, RequestApplicability::Supported);
        assert_eq!(aggregate.processing, ExpectedProcessing::Complete);
        assert_eq!(
            aggregate.qualification,
            Some(ExpectedQualification::SyntaxRejected)
        );
        assert_eq!(
            aggregate.implementation_coverage,
...
#[test]
fn locally_definitive_grammar_evidence_survives_a_later_unowned_tail_after_context_is_fixed() {
...
fn unowned_context_cannot_gain_selected_grammar_evidence_by_searching_for_escape_text() {
    ...
    assert_eq!(formed_prefix.subject, GoldRange::new(9, 13));
}
...
fn formed_ues_positional_failures_remain_static_semantics_evidence_not_grammar_evidence() {
    ...
    for (gold_id, subject) in contrasts {
        ...
        assert_eq!(fixture.subject, Some(subject));
        let authored = authored_fragment(fixture.source, subject);
        assert_eq!(
        ...
        assert_eq!(
            formed_unicode_escape_end(fixture.source, subject.start),
            Some(subject.end)
        );
    }
}
...
fn grammar_evidence_validation_dependency_direction_stays_candidate_independent() {
    let validation_source = include_str!("grammar_evidence.rs");
    for forbidden in [
