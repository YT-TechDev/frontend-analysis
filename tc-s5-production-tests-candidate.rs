...
use super::result::{
    freeze, HtmlAuthoredSource, HtmlConstructedNodeId, HtmlDocumentShellAnalysis,
    HtmlDocumentShellParts, HtmlElement, HtmlParagraphClosure, HtmlParagraphElementOrigin,
    HtmlParagraphSynthesisCause, HtmlSelectedOrdinaryElementName, HtmlShellElementName,
    HtmlShellElementOrigin, HtmlTreeAction, HtmlTreeActionKind, HtmlTreeCapability,
...
use super::session::{admit, token_trigger, HtmlTreeSession, TokenOutcome};
...
fn paragraph_nodes(analysis: &HtmlDocumentShellAnalysis) -> Vec<&HtmlTreeNode> {
    analysis
        ...
        .into_iter()
        .filter(|node| matches!(node.kind(), HtmlTreeNodeKind::Element(HtmlElement::Paragraph(_))))
        .collect()
}
...
fn assert_unsupported(
    ...
    assert_eq!(unsupported.trigger().token_index(), refused_token, "{source:?}");
    assert_eq!(trigger_span(unsupported.trigger()), Some(refused_span), "{source:?}");
    assert_eq!(analysis.coverage().committed_end(), committed_end, "{source:?}");
    assert_eq!(analysis.coverage().processed_tokens(), processed_tokens, "{source:?}");
        ...
        .map(|node| {
            let meaning = match node.kind() {
                ...
                HtmlTreeNodeKind::Element(HtmlElement::SelectedOrdinary(selected)) => {
                    ...
                    NodeMeaning::Selected(name, span(selected.complete()), span(selected.raw_name()))
                }
                HtmlTreeNodeKind::Element(HtmlElement::Paragraph(paragraph)) => {
...
fn p1_authored_lifecycle_text_parentage_and_exact_matching_closure() {
    ...
    assert_eq!(diagnostic_signatures(&analysis), vec![DiagnosticSignature {
        code: HtmlTreeDiagnosticCode::MissingDoctype,
        recovery: HtmlTreeRecovery::ContinuedInQuirksDocumentMode,
        token_index: 0,
        trigger: Some((0, 6)),
    ...
    assert_eq!(paragraph_authored_origin(p), (SourceId::new(11), (6, 9), (7, 8)));
...
fn p4_p5_block_starts_close_p_before_predecessor_insertion() {
    for (id, block, trigger) in [
        ...
        assert_eq!(diagnostic_signatures(&analysis), vec![missing_doctype()], "{id}");
        let p = paragraph_nodes(&analysis)[0];
        ...
        assert!(close_index < insert_index, "{id}: P closes before block insertion");
        assert_eq!(trigger_span(analysis.actions()[close_index].trigger()), Some(trigger));
        assert_eq!(
...
fn p7_unmatched_end_has_one_extra_diagnostic_source_less_synthesis_and_close() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            ParagraphEvent {
                kind: ParagraphEventKind::Closed(
                    HtmlParagraphClosure::UnmatchedEndTagSynthesized,
                ),
                node: p.id(),
                token_index: 1,
                trigger: Some((6, 10)),
...
fn p10_p_only_eof_is_complete_open_and_has_no_p_specific_eof_event() {
    ...
    assert_eq!(analysis.coverage().committed_end(), fixture("P10").source.len());
}
...
fn p11_open_block_eof_keeps_only_the_predecessor_block_diagnostic() {
    ...
    assert_eq!(diagnostics[1].code, HtmlTreeDiagnosticCode::OpenSelectedOrdinaryElementAtEndOfFile);
    assert_eq!(diagnostics[1].recovery, HtmlTreeRecovery::StoppedParsingWithOpenSelectedOrdinaryElements);
    assert!(diagnostics[1].trigger.is_none(), "EOF carries no fabricated authored range");
    assert_eq!(paragraph_nodes(&analysis).len(), 1);
    assert!(paragraph_events(&analysis)
        .iter()
...
fn p18_predecessor_div_section_lifecycle_has_exact_zero_p_delta() {
    ...
    assert_eq!(selected_nodes(&analysis, HtmlSelectedOrdinaryElementName::Div).len(), 1);
    assert_eq!(
...
fn p19_tc_s4_heterogeneous_recovery_remains_separate_from_p() {
    ...
    assert_eq!(diagnostics.len(), 2);
    assert_eq!(diagnostics[0], missing_doctype());
    assert_eq!(diagnostics[1].code, HtmlTreeDiagnosticCode::MisnestedSelectedOrdinaryEndTag);
    assert_eq!(
...
fn semantic_meaning_survives_private_storage_reversal() {
    for id in ["P3", "P7", "P8", "P9", "P19"] {
        ...
        assert_eq!(diagnostic_signatures(&reversed), expected_diagnostics, "{id}");
        assert_eq!(
            (
...
fn lower_layer_incompleteness_is_never_upgraded() {
    ...
    let result = construct_html_document_shell(
        &source,
...
fn freeze_rejects_unmatched_p_diagnostic_and_synthesis_closure_corruption() {
    ...
    let (source, run, mut parts) = prepare_complete_parts(fixture("P7").source);
    parts.actions.retain(|action| !matches!(
        action.kind(),
        HtmlTreeActionKind::ClosedParagraphElement {
